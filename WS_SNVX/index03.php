...
$app->get('/v1/BuscarRuexNit/:ruex/:nit',$authentication, function($ruex_consulta, $nit_consulta) use($db) {
    $db = new PDO('pgsql:host=' . BD_SERVIDOR . ';dbname=' . BD_NOMBRE, BD_USUARIO, BD_PASSWORD);
...
where e.ruex = :param1 and e.nit = :param2");        
    ...
    if($resultados)
		...
		//echo json_encode($resultados, JSON_UNESCAPED_UNICODE);
                ...
                echo json_encode(array('existe' => $resultados[0]['existe'],'fecha_asignacion_ruex' => $resultados[0]['fecha_asignacion_ruex'], 'fecha_renovacion_ruex' => $resultados[0]['fecha_renovacion_ruex'],'ruex_pdf' => pdfbase64($ruex_consulta)));
                //echo pdfbase64('123');
            }
            else
            {
