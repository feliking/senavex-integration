...
$app->post('/v1/BuscarRuexNit', function() use($db) {
    $db = new PDO('pgsql:host=' . BD_SERVIDOR . ';dbname=' . BD_NOMBRE, BD_USUARIO, BD_PASSWORD);
        $ruex_consulta= $_REQUEST['ruex_consulta'];
        $nit_consulta= $_REQUEST['nit_consulta'];
    $consulta = $db->prepare("select * from empresa e where e.ruex = :param1 and e.nit = :param2");        
    ...
    if($resultados)
        {
            ...
            {
		//echo json_encode($resultados, JSON_UNESCAPED_UNICODE);
                //print_r ($resultados[0]['existe']);
                
                
                $pdfconsulta=pdfbase64($ruex_consulta); 
                
                echo json_encode(array('existe' => $resultados[0]['existe'],'fecha_asignacion_ruex' => $resultados[0]['fecha_asignacion_ruex'], 'fecha_renovacion_ruex' => $resultados[0]['fecha_renovacion_ruex'],'ruex_pdf' =>$pdfconsulta ));
                //echo pdfbase64('123');
            }
            else
            {
        ...
        {
            echo json_encode(array('estado' => false, 'mensaje' => 'NO SE ENCONTRARON REGISTROS'));
        }
});
// buscar Datos por RUEX y NIT
$app->post('/v2/BuscarRuexNit', $authentication, function() use($db) {
    $db = new PDO('pgsql:host=' . BD_SERVIDOR . ';dbname=' . BD_NOMBRE, BD_USUARIO, BD_PASSWORD);
    $ruex_consulta= $_REQUEST['ruex_consulta'];
    $nit_consulta= $_REQUEST['nit_consulta'];
    $consulta = $db->prepare("select * from empresa e where e.ruex = :param1 and e.nit = :param2");        
    ...
    if($resultados)
        {
            $consulta = $db->prepare("select e.ruex  as codigo, 'SERVICIO NACIONAL DE VERIFICACION DE EXPORTACIONES' as institucion, 'REGISTRO UNICO DE EXPORTADORES' as nombre_certificado, e.fecha_asignacion_ruex  as f_incripcion, e.fecha_renovacion_ruex  as f_vigencia, e.fecha_asignacion_ruex as f_ult_renovacion, e.estado as estado, e.idactividad_economica as tipo_actividad_economica, e.idrubro_exportaciones as rubro_exportacion, e.afiliaciones as afiliaciones from empresa e where e.ruex = :param1 and e.nit = :param2");
            $consulta->execute(array(':param1' => $ruex_consulta,':param2' => $nit_consulta ));
            $resultados = $consulta->fetchAll(PDO::FETCH_ASSOC);
            echo json_encode($resultados);
...
$app->post('/v1/BuscarNit', $authentication, function() use($db,$app) {
    $db = new PDO('pgsql:host=' . BD_SERVIDOR . ';dbname=' . BD_NOMBRE, BD_USUARIO, BD_PASSWORD);
    $nit_consulta= $_REQUEST['nit_consulta'];
    $consulta = $db->prepare("select e.direccion,e.ruex, e.razon_social,e.nit, ae.descripcion as actividad_de_empresa, e.idrubro_exportaciones, te.descripcion as tipo_de_empresa, 
                            e.email, e.estado,(p.paterno || ' ' || p.materno || ' ' || p.nombres) as rep_legal
                            from empresa e inner join direccion d on (e.direccion = d.id_direccion::text)
                            inner join categoria_empresa ce on (e.idcategoria_empresa = ce.id_categoria_empresa)
                            inner join actividad_economica ae on (ae.id_actividad_economica = e.idactividad_economica)
                            inner join tipo_empresa te on (e.idtipo_empresa = te.id_tipo_empresa)
                            inner join persona p on (p.id_persona = e.id_representante_legal)
                            inner join estado_empresas ee on (ee.estado = e.estado)
                            where e.fecha_renovacion_ruex>=NOW() and (e.estado!=10 and e.estado!=3 and e.estado!=0 and e.estado!=11 and e.estado!=12 and e.estado!=15 and e.estado!=16) and e.nit=:param1");        

    $consulta->execute(array(':param1' => $nit_consulta ));
    $resultados = $consulta->fetchAll(PDO::FETCH_ASSOC);

    if($resultados)
        {
            //$consulta = $db->prepare("select e.ruex  as codigo, 'SERVICIO NACIONAL DE VERIFICACION DE EXPORTACIONES' as institucion, 'REGISTRO UNICO DE EXPORTADORES' as nombre_certificado, e.fecha_asignacion_ruex  as f_incripcion, e.fecha_renovacion_ruex  as f_vigencia, e.fecha_asignacion_ruex as f_ult_renovacion, e.estado as estado, e.idactividad_economica as tipo_actividad_economica, e.idrubro_exportaciones as rubro_exportacion, e.afiliaciones as afiliaciones from empresa e where e.ruex = :param1 and e.nit = :param2");
            //$consulta->execute(array(':param2' => $nit_consulta ));
            //$resultados = $consulta->fetchAll(PDO::FETCH_ASSOC);
            //echo json_encode($resultados);
            foreach ($resultados as $r) {
                //echo $r['idrubro_exportaciones'];
                $rubros=explode(",", $r['idrubro_exportaciones']);
                $rubros_desc='';
                foreach($rubros as $rub) 
                {
                    $consulta_r= $db->prepare("select * from rubro_exportaciones where id_rubro_exportaciones=:paramr");
                    $consulta_r->execute(array(':paramr' => $rub ));
                    $resultados_r = $consulta_r->fetchAll(PDO::FETCH_ASSOC);
                    foreach ($resultados_r as $ru) {
                        $rubros_desc.= ($ru['descripcion']).'|';
                    }
                }
                echo json_encode(array('Codigo' => $r['ruex'],'nit'=>$r['nit'],'razonSocial'=>$r['razon_social'],'descripcionTipoUnidadEconomica'=>$r['tipo_de_empresa'],'$rubrosDescripcion'=>substr($rubros_desc, 0, -1),'representateLegal'=>$r['rep_legal']));
        ...
        {
            $consulta1 = $db->prepare("select * from empresa e where e.nit = :param1 and (e.estado=10 or e.estado=14)");
            $consulta1->execute(array(':param1' => $nit_consulta ));
            $resultados1 = $consulta->fetchAll(PDO::FETCH_ASSOC);
            if($resultados)
            {
                echo json_encode(array('mensaje' => 'RUEX no vigente'));
                //mixed http_response_code(400);
                $app->response->setStatus(400);
            }
            else
            {
                $app->response->setStatus(400);
                echo json_encode(array('mensaje' => 'RUEX inexistente'));
                //mixed http_response_code(401);
