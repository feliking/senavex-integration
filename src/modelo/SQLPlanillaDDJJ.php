...
//        return $planilladdjj->findAll($sql);
    ...
    public function getListarDdjjOrigen_Tipo_Fecha(PlanillaDDJJ $planillaDDJJ, $fecha_fin) {
        ...
        $sql = "select pd.descripcion, pd.numero_ddjj, pd.fecha_registro, pd.fecha_revision, pd.fecha_vencimiento, pd.fecha_baja, pd.observacion, e.ruex, e.razon_social, pac.descripcion as acuerdo, pd.id_nandina, pde.codigo as nandina, pdee.codigo as naladisa, pco.descripcion as criterio_origen, pda.complemento, r.ciudad, ce.descripcion as categoria_empresa, CONCAT(per.nombres, ' ', per.paterno, ' ', per.materno) as certificador
from planilla_ddjj pd, empresa e, planilla_acuerdo pac, planilla_ddjj_acuerdo pda left join planilla_detalle_arancel pdee on pda.id_detalle_arancel = pdee.id_detalle_arancel, planilla_criterio_origen pco, planilla_detalle_arancel pde, regional r, categoria_empresa ce, persona per 
where pd.id_empresa = e.id_empresa and r.id_regional = pd.id_regional and pda.id_planilla_ddjj = pd.id_planilla_ddjj and e.idcategoria_empresa = ce.id_categoria_empresa and pda.id_acuerdo = pac.id_acuerdo and pd.id_estado = 1 and pd.id_asistente_revision = per.id_persona  
...
where p.id_empresa = ". $id_empresa ." and p.id_estado = 1 and p.fecha_vencimiento >= now() and pde.id_detalle_arancel = p.id_nandina and ptd.id_planilla_tipo_ddjj = pac.id_tipo_ddjj and p.fecha_baja is null 
