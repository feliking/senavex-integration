...
class PDF extends tFPDF
{
    ...
    function Pie($codigo_control, $fecha_limite)
    {
        ...
        $this->Cell(190,4,utf8_decode('"ESTA FACTURA CONTRIBUYE AL DESARROLLO DEL PA�?S, EL USO IL�?CITO DE ÉSTA SER�? SANCIONADO DE ACUERDO A LEY"'),0,0,'C');
        $this->Ln();
        $this->SetFont('Arial','',7);
...
//        $this->SetFillColor(255);
       ...
       // $this->Image('http://chart.apis.google.com/chart?chs=40x40&cht=qr&chl='.$texto_qr,140,215,40,0,'PNG');
        if(!empty($texto_qr)){
            ...
            //$this->Image('https://api.qrserver.com/v1/create-qr-code/?size=50x50&data='.$texto_qr,$this->getX1()+74,174,30,0,'PNG');
        }
        //detalle cabecera
//        $this->SetFillColor(176,176,176);
