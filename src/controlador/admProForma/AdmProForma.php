...
include_once(PATH_MODELO . DS . 'SQLFacturaSenavexPersonaImport.php');
...
include_once(PATH_TABLA . DS . 'FacturaSenavexPersonaImport.php');
...
//            print('<pre>'.print_r($this->loadServicios($_REQUEST['']),true).'</pre>');
        ...
        if($_REQUEST['tarea']=='factura_manual_importaciones')
        {
                       //print('<pre>'.print_r($_SESSION,true).'</pre>');
            $regional = new Regional();
            $sqlRegional = new SQLRegional();
            ...
            $listaRegionales1 = $sqlRegional->getListarRegionales($regional,false);
            $listaRegionales = $sqlDepartamento->getListarDepartamento($departamento);
            $listaCategorias = $sqlCategoriaEmpresa->getListarCategoriaEmpresa($categoriaEmpresa);
             // var_dump($listaCategorias);  die;
            
            $servicio=new Servicio();
            $sqlServicio=new SQLServicio();
            $servicio->setId_Categoria_Servicio(8);
            $lista=$sqlServicio->getListaServiciosPorCategoria($servicio);
            //$fmsucursal = $_SESSION['id_empresa_persona'];
            $ep=new EmpresaPersona();
            $sqlep = new SQLEmpresaPersona();
            ...
            $regional->setId_regional($fmsucursal);
            $regional=$sqlRegional->getBuscarRegionalPorId($regional);
            $vista->assign('servicios_imp',$lista);
            $vista->assign('listaRegionales',$listaRegionales1);
            $vista->assign('listaDepartamento',$listaRegionales);
            $vista->assign('listaCategorias',$listaCategorias);
            $vista->assign('fmsucursal_imp',$fmsucursal);
            $vista->assign('fmsucursalnombre_imp',$regional->getCiudad());
            $vista->display("ProForma/factura_manual_import.tpl");
        }
        ...
        if($_REQUEST['tarea']=='factura_senavex_persona_import'){
            $facturaSenavexPersonaImport=new FacturaSenavexPersonaImport();
            $sqlfacturaSenavexPersonaImport = new SQLFacturaSenavexPersonaImport();
            $facturaSenavexPersonaImport->setCi($_REQUEST['ci_imp']);
            $facturaSenavexPersonaImport=$sqlfacturaSenavexPersonaImport->getPersonaPorCI($facturaSenavexPersonaImport);
            if(empty($facturaSenavexPersonaImport)==true){
                echo '-1';
                //print('<pre>'.print_r($_REQUEST,true).'</pre>');
            }else{
                echo '[';
                echo '{"id":"'.$facturaSenavexPersonaImport->getId_factura_senavex_persona_import().'",'
                    .'"ci":"'.$facturaSenavexPersonaImport->getCi().'",'
                    .'"nombre":"'.$facturaSenavexPersonaImport->getNombres().' '.$facturaSenavexPersonaImport->getApellido_paterno().' '.$facturaSenavexPersonaImport->getApellido_materno().'"}';
                echo ']';
            }
            exit;
        }
        ...
        if($_REQUEST['tarea']=='factura_senavex_empresa_import'){
            $facturaSenavexEmpresaImport=new FacturaSenavexEmpresaImport();
            $sqlFacturaSenavexEmpresaImport = new SQLFacturaSenavexEmpresaImport();
            $facturaSenavexEmpresaImport->setNit($_REQUEST['nit_imp']);
            $facturaSenavexEmpresaImport=$sqlFacturaSenavexEmpresaImport->getEmpresaPorNIT($facturaSenavexEmpresaImport);
            //print('<pre>'.print_r($_REQUEST,true).'</pre>'); 
            if(empty($facturaSenavexEmpresaImport)==true){
                echo '-1';
            }else{
                echo '[';
                echo '{"id":"'.$facturaSenavexEmpresaImport->getId_factura_senavex_empresa_import().'",'
                    .'"nombre":"'.$facturaSenavexEmpresaImport->getNombre().'"}';
                echo ']';
            }
            exit;
        }
	    ...
	    $facturaSenavexManual->setId_tipo(1);
...
//                        print('<pre>'.print_r('['.$index.']'.$lista[$index].' = '.$_REQUEST[$lista[$index]], true).'</pre>');
        ...
        //*****************************************************
        if($_REQUEST['tarea']=='generar_factura_manual_import'){
           ...
           // $facturaSenavexManual->setFecha_limite(date('Y-m-d', strtotime("+60 days")));
            $facturaSenavexManual->setId_asistente($_SESSION['id_persona']);
            $facturaSenavexManual->setId_regional($_REQUEST['fmsucursal_imp']);
            //$facturaSenavexManual->setNumero_autorizacion(AUTORIZACION);
            // como es facturacion importacion pondremos el nit y la empresa persona 
                $empresa_x = $this->getEmpresa($_REQUEST['empresa_id_imp']);
                $persona_x = $this->getPersona($_REQUEST['persona_id_imp']);
                $facturaSenavexManual->setId_empresa($_REQUEST['empresa_id_imp']);
                $facturaSenavexManual->setId_persona($_REQUEST['persona_id_imp']);
            $facturaSenavexManual->setVortex(2);
            $facturaSenavexManual->setTotal($_REQUEST['total_facturar']);
            $facturaSenavexManual->setNumero_factura(-1);
            $facturaSenavexManual->setNumero_recibo(-1);
	     ...
	     $val = explode("-", $lista[3]);
            ...
            if ($sqlFacturaSenavexManual->Save($facturaSenavexManual))
            {$swfactura=true;}
            
            
            //obtenemos el id de la factura generada
            $id_factura_senavex_manual = $facturaSenavexManual->getId_factura_senavex_manual(); 

            // Grabamos los depositos
            $suma=0;
            for ($index = 3; $index < count($lista); $index++) {
                $val = explode("-", $lista[$index]);
                $num = explode("_",$val[1]);
                ...
                if($deposito[0]=='numero'){
                    ...
                    $fmDeposito->setEstado(0);
                        ...
                        $fmDeposito->setId_Empresa($_REQUEST['empresa_id_imp']);
                        $fmDeposito->setId_Persona($_REQUEST['persona_id_imp']);
                ...
                // hasta aqui grabamos deposito
               
                if($val[0]=='serexp'){
//                    print('<pre>'.print_r('entro a serexp', true).'</pre>');
//                    print('<pre>'.print_r('['.$index.']'.$lista[$index].' = '.$_REQUEST[$lista[$index]], true).'</pre>');
                    $facturaSenavexManual->setVortex(0);
                    $sqlFacturaSenavexManual->Save($facturaSenavexManual);
                    ...
                    $facturaSenavexManualDetalle->setId_servicio($servicioExportador->getId_servicio_exportador());
                    $facturaSenavexManualDetalle->setCantidad(1);
                    $facturaSenavexManualDetalle->setVortex(2);
                    $facturaSenavexManualDetalle->setPrecio($servicioExportador->getCosto_Actual());
                    $facturaSenavexManualDetalle->setId_factura_senavex_manual($id_factura_senavex_manual);
                    $sqlFacturaSenavexManualDetalle->Save($facturaSenavexManualDetalle);
                    $swfactura=true;
...
//                    print('<pre>'.print_r('['.$index.']'.$lista[$index].' = '.$_REQUEST[$lista[$index]], true).'</pre>');
                    ...
                    if($id_servicio >= 75 && $id_servicio <= 91){ 
                        $swfactura=true;
                        $index+= 2;
                        $index1 = $index;
                        $var1= explode("-", $lista[$index]);
                        while($var1[0]=='cantidadventa'){
                            $icontrol = $_REQUEST['icontrol-'.$var1[1].'-'.$var1[2]];
                            $fcontrol = $_REQUEST['fcontrol-'.$var1[1].'-'.$var1[2]];
                            $cantidadDif = intval($fcontrol) - intval($icontrol)+1;
                            // $diferencia = intval($fcontrol) - intval($icontrol);
            ...
            if($swfactura){
               if($_REQUEST['chx_fmcontingencia_imp']!='on'){
                    //$fcorrelativo->setId_factura_senavex_manual_correlativo(1);
                    $fcorrelativo->setActivo(1);
                    $fcorrelativo = $sqlFCorrelativo->getCorrelativoActivo($fcorrelativo);
                    $fcorrelativo->setCorrelativo($fcorrelativo->getCorrelativo()+1);
                    ...
                    $facturaSenavexManual->setNumero_factura($_REQUEST['fmc_nro_imp']);
                    $facturaSenavexManual->setFecha_emision($_REQUEST['fmc_fecha_imp']);
                    $facturaSenavexManual->setFecha_limite($_REQUEST['fmc_fecha_limite_imp']);
                    $facturaSenavexManual->setNumero_autorizacion($_REQUEST['fmc_autorizacion_imp']);
                    $facturaSenavexManual->setEstado(5);
                }
            }
            if($swrecibo){
...
//                print('<pre>'.print_r($value,true).'</pre>');
                ...
                } elseif($value->getVortex()==2){
                    
                    $facturaSenavexEmpresaImport->setId_factura_senavex_empresa_import($value->getId_empresa());
                    $empresa=$sqlFacturaSenavexEmpresaImport->getEmpresa($facturaSenavexEmpresaImport);
                    $fmempresa->setNombre($empresa->getNombre());
                }else{
...
////                print('<pre>'.print_r($empresa,true).'</pre>');exit;
        ...
        if($_REQUEST['tarea']=='editar_factura_manual'){
            ...
            }elseif($facturaSenavexManual->getVortex()==2){
                    
                $facturaSenavexEmpresaImport->setId_factura_senavex_empresa_import($facturaSenavexManual->getId_empresa());
                $empresa=$sqlFacturaSenavexEmpresaImport->getEmpresa($facturaSenavexEmpresaImport);
                $facturaSenavexEmpresa->setNombre($empresa->getNombre());
                $facturaSenavexEmpresa->setNit($empresa->getNit());
            }else{
...
//***************************FACTURA AUTOMATICA**********************************//
    ...
    private function verFactura($id_factura_senavex_manual,$estado){
        ...
        if($facturaSenavexManual->getVortex()==1){
            ...
            if($facturaSenavexManual->getVortex()==2){
            $facturaSenavexEmpresaImport->setId_factura_senavex_empresa_import($facturaSenavexManual->getId_empresa());
            $facturaSenavexEmpresaImport = $sqlFacturaSenavexEmpresaImport->getEmpresa($facturaSenavexEmpresaImport);            
            }
            else{
                $facturaSenavexEmpresa->setId_factura_senavex_empresa($facturaSenavexManual->getId_empresa());
                $facturaSenavexEmpresa=$sqlFacturaSenavexEmpresa->getEmpresa($facturaSenavexEmpresa);      
        ...
        if(count($listaServicios)>0)
        {
            if($facturaSenavexManual->getVortex()==2)
                {
                $nit = $facturaSenavexEmpresaImport->getNit();
                $empresa_nombre = $facturaSenavexEmpresaImport->getNombre();
                }
                else{
