...
//            print('<pre>'.print_r($contenido,true).'</pre>');
        ...
        if($_REQUEST['tarea']=='Cierres'){
            ...
            if($tipo_reporte == 1){
                $this->CierreGeneral($_SESSION['id_empresa_persona'], $fecha_ini, $fecha_fin, $_REQUEST['regional_reporte']);
            }
            if($tipo_reporte == 2){
            ...
            if($tipo_reporte == 5){
                $this->getRecaudaciones($_SESSION['id_empresa_persona'], $fecha_ini, $fecha_fin, $_REQUEST['regional_reporte']);
            }
            if($tipo_reporte == 6){
...
//                $this->getReporteEmisiones($_SESSION['id_empresa_persona'], $fecha_ini, $fecha_fin, $_REQUEST['regional_reporte']);
    ...
    public function CierreGeneral($id_empresa_persona, $fecha_ini, $fecha_fin, $id_regional){
...
//         print('<pre>'.print_r($regional,true).'</pre>');
        ...
        $servicio->setId_Categoria_Servicio('6,7');
        ...
        $fsm->setEstado(2);
        $facturas1 = $sqlFSM->ListFacturaManualFecha2($fsm, $fecha_ini, $fecha_fin);
        $fsm->setId_regional($regional->getId_regional());
        $fsm->setEstado(5);
        $facturas2 = $sqlFSM->ListFacturaManualFecha2($fsm, $fecha_ini, $fecha_fin);
        ...
        foreach ($listaServicios as $value) {
            ...
            $listaDetalle = $sqlFSMDetalle->getListFacturaDetallePorServicioFecha($fsmDetalle,$regional->getId_regional(), $fecha_ini, $fecha_fin);
            $suma = 0;
            $del = -1;
            $al = -1;
            $cantidad = 0;
//                print('<pre>'.print_r($listaDetalle,true).'</pre>');
            if(count($listaDetalle)>0){
                foreach ($listaDetalle as $value2) {
                    ...
                    if(($value->getId_servicio()>= 11 && $value->getId_servicio() <= 46)){
...
//print("<pre>".print_r($_REQUEST, true)."</pre>");
    ...
    public function getRecaudaciones($id_empresa_persona, $fecha_ini, $fecha_fin, $id_regional){
//        print('<pre>'.print_r('<h1>REPORTE DE RECAUDACIONES DE SERVICIOS PRESTADOS EN PROCESO...</h1>',true).'</pre>');
        ...
        $sqlReportesEstadisticas = new SQLReportesEstadisticas();
        if($id_regional!=null && $id_regional!='-1' && $id_regional!=''){
            $lista1 = $sqlReportesEstadisticas->recaudaciones($id_regional, 2, $fecha_ini, $fecha_fin);
            $lista2 = $sqlReportesEstadisticas->recaudaciones($id_regional, 5, $fecha_ini, $fecha_fin); 
        }else{
            $lista1 = $sqlReportesEstadisticas->recaudaciones($empresa_persona->getId_regional(), 2, $fecha_ini, $fecha_fin);
            $lista2 = $sqlReportesEstadisticas->recaudaciones($empresa_persona->getId_regional(), 5, $fecha_ini, $fecha_fin); 
        }
        ...
        $servicios->setId_Categoria_Servicio("6,7");
    ...
    public function getReporteDdjjOrigen($id_empresa_persona, $fecha_ini, $fecha_fin, $id_regional){
        ...
        try {
            ...
            foreach ($list as $value) {
                ...
                $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(3 ,$row + $index, $value['acuerdo']);
		...
		$objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(8 ,$row + $index, $value['numero_ddjj']);
                ...
                $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(13 ,$row + $index, $meses[intval(date('m',strtotime($value['fecha_revision'])))]);
                $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(14 ,$row + $index, date('Y',strtotime($value['fecha_revision'])));
                $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(15 ,$row + $index, $value['ciudad']);
                $index++;
            }
	    $fila = $row + $index -1;
...
//            print("<pre>".print_r($_REQUEST, true)."</pre>");
            ...
            foreach ($list as $value) {
                ...
                $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(15 ,$row + $index, $value['codigo_pais']);
                $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(16 ,$row + $index, $value['pais']);
                $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(17 ,$row + $index, $value['certificador']);
                $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(18 ,$row + $index, $value['tipoemision']);
                $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(19 ,$row + $index, $value['observacion']);
                $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(20 ,$row + $index, $value['ciudad']);
                $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(21 ,$row + $index, $meses[intval(date('m',strtotime($value['fecha_revision'])))]);
                $objPHPExcel->setActiveSheetIndex($sheet)->setCellValueByColumnAndRow(22 ,$row + $index, date('Y',strtotime($value['fecha_revision'])));
                $index++;
            }
	    $fila = $row + $index -1;
