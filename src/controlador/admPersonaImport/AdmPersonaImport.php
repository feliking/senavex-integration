...
class AdmPersonaImport extends Principal {

  public function AdmPersonaImport() {
...
//                    else echo '[{"suceso":"0","msg":"Datos guardados Correctamente."}]';
    ...
    if($_REQUEST['tarea']=='agregarPersonaImport'){
        //print('<pre>'.print_r($_REQUEST,true).'</pre>');
        $FSPersona = new FacturaSenavexPersonaImport();
        $sqlFSPersona = new SQLFacturaSenavexPersonaImport();
...
//        print('<pre>'.print_r($FSPersona,true).'</pre>');
        if(empty($FSPersona)==true){
            ...
            echo '[';
            echo '{"id":"'.$FSPersona->getId_factura_senavex_persona_import().'",'
                .'"ci":"'.$FSPersona->getCi().'",'
