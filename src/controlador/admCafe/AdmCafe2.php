...
class AdmCafe_2 extends Principal {
    public function AdmCafe() 
    {
        ...
        if($_REQUEST['tarea']=='validate_cafe')
        {
            ...
            if($_REQUEST['validado']=='1')
            {
                ...
                $cafeBorrador->setNro_interno($cafeCertificado->getId_cafe_certificado());
        ...
        if($_REQUEST['tarea']=='save_cafe')
        {
            ...
            $cafeBorrador->setCafe_importador($_REQUEST["idimportador"]);
            ...
            $cafeBorrador->setId_servicio_exportador($servicioExportador->getId_servicio_exportador());
        ...
        if($_REQUEST['tarea']=='guardarImportador')
        {
            $cafeImportador=new CafeImportador();
            $sqlCafeImportador=new SQLCafeImportador();
            $cafeImportador->setNombre($_REQUEST['nombre_importador']);
        ...
        if($_REQUEST['tarea']=='listarPaises')
        {
            ...
            foreach ($resultado as $datos){
                $strJson .= '{"clave_oic":"' . $datos->getClave_oic() .
