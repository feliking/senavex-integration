<form name="alta_ddjj" id="alta_ddjj" method="post"  action="index.php">

<div class="row-fluid  form" id="enviardeclaracionjurada">
    <div class="row-fluid " >
        <div class="span12" >
            <div class="k-block fadein">
                <div class="k-header">
                    <div class="row-fluid form">
                        <div class="span12">
                            <div class="titulonegro" > Nueva Declaración Jurada</div> 
                        </div>
                    </div>  
                </div>
                <div class="k-cuerpo">
                    <!-- ACUERDOS COMERCIALES -->
                    <fieldset>
                        <legend align="center"><b>ACUERDOS COMERCIALES</b></legend>
                        <div class="row-fluid form" style="text-align: left;" >
                            <div class="span2" >
                                <input type="checkbox" name="lista_acuerdos[]" id="check_ace22" onclick="aparecer_fob(check_ace22);cuentaAcuerdos(check_ace22);" value="ace22">ACE-22
                            </div>
                            <div class="span2" >
                                <input type="checkbox" name="lista_acuerdos[]" id="check_ace47" onclick="aparecer_fob(check_ace47);aparecer_Naladisa2007(check_ace47);cuentaAcuerdos(check_ace47);" value="ace47">ACE-47
                            </div>
                            <div class="span2" >
                                <input type="checkbox" name="lista_acuerdos[]" id="check_arpar4" onclick="aparecer_fob(check_arpar4);cuentaAcuerdos(check_arpar4);" value="arpar4">AR.PAR N°4
                            </div>
                            <div class="span2" >
                                <input type="checkbox" name="lista_acuerdos[]" id="check_aapag" onclick="aparecer_fob(check_aapag);cuentaAcuerdos(check_aapag);" value="aapag">AAP.AG N°2(Semillas)
                            </div>
                            <div class="span2" >
                                <input type="checkbox" name="lista_acuerdos[]" id="sgp_tp" onclick="aparecer_exwork(sgp_tp);cuentaAcuerdos(sgp_tp);" value="tp">TERCEROS PAISES
                            </div>
                        </div>
                        <div class="row-fluid form" style="text-align: left;">
                            <div class="span2" >
                                <input type="checkbox" name="lista_acuerdos[]" id="check_can" onclick="aparecer_fob(check_can);cuentaAcuerdos(check_can);" value="can">CAN
                            </div>
                            <div class="span2" >
                                <input type="checkbox" name="lista_acuerdos[]" id="check_mercosur" onclick="aparecer_fob(check_mercosur);cuentaAcuerdos(check_mercosur);" value="mercosur">MERCOSUR
                            </div>
                            <div class="span2" >
                                <input type="checkbox" name="lista_acuerdos[]" id="check_venezuela" onclick="aparecer_fob(check_venezuela);cuentaAcuerdos(check_venezuela);" value="venezuela">VENEZUELA
                            </div>
                            <div class="span2" >
                                <input type="checkbox" name="lista_acuerdos[]" id="check_arampanama" onclick="aparecer_fob(check_arampanama); aparecer_Naladisa2007(check_arampanama);cuentaAcuerdos(check_arampanama);" value="arampanama">ARAM. PANAMÁ
                            </div>
                            <div class="span2" >
                                <input type="checkbox" name="check_sgt" id="check_sgt">SGP
                            </div>
                        </div>
                        <div class="row-fluid form" style="display:none; text-align: left;" id="div_sgt">
                            <fieldset><legend>REGIMENES PREFERENCIALES EN SGP</legend>
                            <div class="span3 " >
                            <input type="checkbox" name="lista_acuerdos[]" id="sgp_canada" onclick="aparecer_exwork(sgp_canada);cuentaAcuerdos(sgp_canada);" value="canada">CANADA
                            </div>
                            <div class="span3 " >
                            <input type="checkbox" name="lista_acuerdos[]" id="sgp_suiza" onclick="aparecer_exwork(sgp_suiza);cuentaAcuerdos(sgp_suiza);" value="suiza">SUIZA
                            </div>
                            <div class="span3 " >
                            <input type="checkbox" name="lista_acuerdos[]" id="sgp_noruega" onclick="aparecer_exwork(sgp_noruega);cuentaAcuerdos(sgp_noruega);" value="noruega">NORUEGA
                            </div>
                            <div class="span3 " >
                            <input type="checkbox" name="lista_acuerdos[]" id="sgp_japon" onclick="aparecer_exwork(sgp_japon);cuentaAcuerdos(sgp_japon);" value="japon">JAPON
                            </div>

                            <div class="span3 " >
                            <input type="checkbox" name="lista_acuerdos[]" id="sgp_zelanda" onclick="aparecer_exwork(sgp_zelanda);cuentaAcuerdos(sgp_zelanda);" value="zelanda">NUEVA ZELANDA
                            </div>
                            <div class="span3 " >
                            <input type="checkbox" name="lista_acuerdos[]" id="sgp_rusia" onclick="aparecer_exwork(sgp_rusia);cuentaAcuerdos(sgp_rusia);" value="rusia">UNION ECONOMICA EUROASIATICA
                            </div>
                            <div class="span3 " >
                            <input type="checkbox" name="lista_acuerdos[]" id="sgp_turquia" onclick="aparecer_exwork(sgp_turquia);cuentaAcuerdos(sgp_turquia);" value="turquia">TURQUÍA
                            </div>
                            <div class="span3 " >
                            <input type="checkbox" name="lista_acuerdos[]" id="sgp_bielorrusia" onclick="aparecer_exwork(sgp_bielorrusia);cuentaAcuerdos(sgp_bielorrusia);" value="bielorrusia">AUSTRALIA
                            </div>
                                
                            <div class="span3 " >
                            <input type="checkbox" name="lista_acuerdos[]" id="sgp_ue" onclick="aparecer_exwork(sgp_ue);cuentaAcuerdos(sgp_ue);" value="ue">UNION EUROPEA
                            </div>
                            <div class="span3 " >
                            <input type="checkbox" name="lista_acuerdos[]" id="sgp_eeuu" onclick="aparecer_exwork(sgp_eeuu);cuentaAcuerdos(sgp_eeuu);" value="eeuu">ESTADOS UNIDOS
                            </div>
                            </fieldset>
                        </div>
                        
                        <div class="row-fluid">
                            <div class="span3" style="display:none; text-align: left;" id="div_fob">
                                <input type="text" placeholder="Valor FOB($us)"  name="valor_fob" id="valor_fob" /> 
                                <div class="span" ><center>
                                    <input type="hidden" name="hiddenvalidatorvalorfob" id="hiddenvalidatorvalorfob" 
                                    data-fobvalidator
                                    data-required-msg="Ingrese el valor FOB de la mercancía" /></center>
                                </div> 
                            </div>
                            <div class="span3" style="display:none; text-align: left;" id="div_exwork">
                                <input type="text" placeholder="Valor ExWork($us)"  name="valor_exwork" id="valor_exwork" /> 
                                <div class="span" ><center>
                                    <input type="hidden" name="hiddenvalidatorvalorexwork" id="hiddenvalidatorvalorexwork" 
                                    data-exworkvalidator
                                    data-required-msg="Ingrese el valor EX-WORK" /></center>
                                </div> 
                            </div>
                        </div>
                    </fieldset>
                    <div class="row-fluid" >
                        <div class="span" ><center>
                            <input type="hidden" name="hiddenvalidator" id="hiddenvalidator" 
                            data-checkvalidator
                            data-required-msg="Por Favor Escoja por lo menos un Acuerdo" /></center>
                        </div> 
                    </div>

                    <!-- Cálculo de lo que se debe pagar -->
                    <div class="row-fluid form" >
                        <div class="span2">
                            <span style="text-align: right; font-weight: bold;">Días para Revisar:</span>
                        </div>
                        <div class="span1">
                            <span style="width: 100%"><input type="text" readonly id="total_ddjj" name="total_ddjj" value="0"></span>
                        </div>
                        <div class="span1">
                            <span style="text-align: left; font-weight: bold;">DÍAS.</span>
                        </div>
                    </div>

                    <fieldset>
                        <legend align="center"><b>NANDINA / NALADISA's</b></legend>
                        <div class="row-fluid form">
                            <div class="span2" align="right">Capítulos</div>
                            <div class="span8">
                                <input id="combocapitulos"  name="combocapitulos" style="width: 100%;"  required validationMessage="Por favor Introduzca el Caítulo de la Mercancía"/>
                            </div>
                        </div>
                        <div class="row-fluid form">
                            <div class="span2" align="right">Subpartida Arancelaria a 10 DÍGITOS</div>
                            <div class="span8">
                                <input id="combonandina"  name="combonandina" style="width: 100%;"  required validationMessage="Por favor Introduzca Subpartida Arancelaria o Descripcion del Producto"/>
                            </div>
                            <div class="span2"></div>
                        </div>
                    
                        <!-- Tratados con NANDINAS y NALADISAS -->
                        <div class="row-fluid" style="display:none; text-align: left;" id="div_naladisa93">
                            <div class="span2" align="right">NALADISA 1993</div>
                            <div class="span8">
                                <input id="naladisa93"  name="naladisa93" style="width: 100%;"/>
                                <div class="span" ><center>
                                    <input type="hidden" name="hiddenvalidatornaladisa93" id="hiddenvalidatornaladisa93" 
                                    data-naladisa93validator
                                    data-required-msg="Ingrese el valor NALADISA 1993 A 8 DÍGITOS" /></center>
                                </div> 
                            </div>
                        </div>
                        <div class="row-fluid" style="display:none; text-align: left;" id="div_naladisa96">
                            <div class="span2" align="right">NALADISA 1996</div>
                            <div class="span8">
                                <input id="naladisa96"  name="naladisa96" style="width: 100%;"/>
                                <div class="span" ><center>
                                    <input type="hidden" name="hiddenvalidatornaladisa96" id="hiddenvalidatornaladisa96" 
                                    data-naladisa96validator
                                    data-required-msg="Ingrese el valor NALADISA 1996 A 8 DÍGITOS" /></center>
                                </div> 
                            </div>
                        </div>
                        <div class="row-fluid"  style="display:none; text-align: left;" id="div_naladisa2007">
                            <div class="span2" align="right">NALADISA 2007</div>
                            <div class="span8">
                                <input id="naladisa2007"  name="naladisa2007" style="width: 100%;"/>
                                <div class="span" ><center>
                                    <input type="hidden" name="hiddenvalidatornaladisa2007" id="hiddenvalidatornaladisa2007" 
                                    data-naladisa2007validator
                                    data-required-msg="Ingrese el valor NALADISA 2007 A 8 DÍGITOS" /></center>
                                </div> 
                            </div>
                        </div>
                        <div class="row-fluid" style="display:none; text-align: left;" id="div_naladisa91">
                            <div class="span2" align="right">NALADISA 1991</div>
                            <div class="span8">
                                <input id="naladisa91"  name="naladisa91" style="width: 100%;"/>
                                <div class="span" ><center>
                                    <input type="hidden" name="hiddenvalidatornaladisa91" id="hiddenvalidatornaladisa91"
                                    data-naladisa91validator
                                    data-required-msg="Ingrese el valor NALADISA 1991 A 8 DÍGITOS" /></center>
                                </div> 
                            </div>
                        </div>
                        <div class="row-fluid" style="display:none; text-align: left;" id="div_naladi83">
                            <div class="span2" align="right">NALADI 1983</div>
                            <div class="span8" >
                                <input id="naladi83"  name="naladi83" style="width: 100%;"/>
                                <div class="span" ><center>
                                    <input type="hidden" name="hiddenvalidatornaladi83" id="hiddenvalidatornaladi83"
                                    data-naladi83validator
                                    data-required-msg="Ingrese el valor NALADI 1983 A 7 DÍGITOS" /></center>
                                </div> 
                            </div>
                        </div>
                    </fieldset>
                    <div class="row-fluid form">
                        <div class="barra">
                        </div>
                    </div>
                    <!-- SELECCIONAR LA FABRICA -->
                    <div class="row-fluid form" >
                        <div class="span8" >
                            <input id="combo_fabricas" name="combo_fabricas" style="width: 100%;" required validationMessage="Por favor Introduzca la Fábrica"/>
                        </div>
                        <div class="span2" style="text-align: left;" >
                            <input id="addfabrica" type="button" value="Agregar Fábrica" class="k-primary" style="width:40"/> 
                        </div>
                    </div>
                    <div class="row-fluid form">
                        <div class="barra">
                        </div>
                    </div>
                    
                    <!-- TABLA PARA LOS REGISTROS EN CASO DE COMERCIALIZADORES -->
                    <div class="row-fluid form" >
                        <div class="span12" style="text-align: left;" >
                            <input type="checkbox" name="check_comercializador" id="check_comercializador">En caso de ser comercializador elegir opción
                        </div>
                    </div>
                    <div class="row-fluid form" style="display:none;" id="div_comercializador">
                        <fieldset>
                            <legend align="center"><b>DATOS DEL PRODUCTOR DE LA MERCADERÍA</b></legend>
                            <div id="tabla_comercializadores" class="fadein" style="width: 100%; font-size: 10px;"></div>
                            <br>
                            <input id="addfilacomercializador" type="button" value="+" class="k-primary" style="width:40" onclick="agregarfilacomercializador();"/> 
                            <input id="elimfilacomercializador" type="button" value="-" class="k-primary" style="width:40" onclick="eliminarfilacomercializador();"/> 
                        </fieldset>
                    </div>
                    <div class="row-fluid form">
                        <div class="barra">
                        </div>
                    </div>
                    
                    <!-- CAMPOS PARA LA DECLARACION JURADA CON LOS DATOS DE LA MERCANCÍA -->
                    <fieldset>
                        <legend align="center"><b>DATOS GENERALES</b></legend>
                        <div class="row-fluid form">
                            <div class="span6" >
                                <input type="text" class="k-textbox" placeholder="Denominación Comercial"  name="denominacion_comercial" id="denominacion_comercial" required validationMessage="Por favor ingrese Denominación Comercial" /> 
                            </div>
                            <div class="span6" >
                                <input type="text" class="k-textbox" placeholder="Nombre Técnico"  name="nombre_tecnico" id="nombre_tecnico" required validationMessage="Por favor ingrese el Nombre Técnico"/>
                            </div>
                        </div>
                        <div class="row-fluid form">
                            <div class="span6" >
                                <input type="text" class="k-textbox" placeholder="Características Técnicas"  name="caracteristicas" id="caracteristicas" required validationMessage="Por favor ingrese Características Técnicas" />
                            </div>
                            <div class="span6" >
                                <input type="text" class="k-textbox" placeholder="Usos y aplicaciones" name="aplicacion" id="aplicacion" required validationMessage="Por favor ingrese sus Usos y aplicaciones"/>
                            </div>
                        </div>
                        <div class="row-fluid form" >
                            <div class="span4 " >
                                <input style="width:100%;" id="unidadmedida" name="unidadmedida" required validationMessage="Por favor Escoja Unidad de Medida">
                            </div>
                            <div class="span4 " >
                                <input type="text" placeholder="Capacidad de Producción Mensual" id="produccion_mensual" name="produccion_mensual" required validationMessage="Coloque la Cap. de Producción Mensual">
                            </div>
                        </div>
                        <div class="row-fluid  form" >
                            <div class="span12" >
                            <input type="text" class="k-textbox "  placeholder="Descripción del Proceso Productivo"  name="procesoproductivo" id="procesoproductivo" required validationMessage="Por favor Especifique el Proceso Productivo"/> 
                            </div>
                        </div>
                    </fieldset>
                    
                    <!-- Tabla de Registros para LOS INSUMOS NACIONALES -->
                    <div class="row-fluid form" id="div_insumosnacionales">
                        <fieldset>
                            <legend align="center"><b>INSUMOS NACIONALES</b></legend>
                            <div id="tabla_insumosnacionales" class="fadein"></div>
                            <br>
                            <input id="addfilainsumosnacionales" type="button" value="+" class="k-primary" style="width:40" onclick="agregarfilainsumosnacionales();"/> 
                            <input id="elimfilainsumosnacionales" type="button" value="-" class="k-primary" style="width:40" onclick="eliminarfilainsumosnacionales();"/> 
                        </fieldset>
                    </div>

                    <!-- Tabla de Registros para LOS INSUMOS IMPORTADOS -->
                    <div class="row-fluid form" >
                        <div class="span12" style="text-align: left;" >
                            <input type="checkbox" name="check_insumosimportados" id="check_insumosimportados">En caso de existir insumos importados elegir opción
                        </div>
                    </div>
                    <!-- Tabla de Registros para los datos del Productor de la mercancia -->
                    <div class="row-fluid form" style="display:none;" id="div_insumosimportados">
                        <fieldset>
                            <legend align="center"><b>INSUMOS IMPORTADOS</b></legend>
                            <div id="tabla_insumosimportados" class="fadein" style="width: 100%;"></div>
                            <br>
                            <input id="addfilainsumosimportados" type="button" value="+" class="k-primary" style="width:40" onclick="agregarfilainsumosimportados();"/> 
                            <input id="elimfilainsumosimportados" type="button" value="-" class="k-primary" style="width:40" onclick="eliminarfilainsumosimportados();"/> 
                        </fieldset>
                    </div>
                    <div class="row-fluid form">
                        <div class="barra">
                        </div>
                    </div>

                    <div class="row-fluid  form" >
                        <div class="span12" style="font-weight: bold; text-align: left;">Indique si los productos son elaborados en zonas especiales o si se benefician de incentivos territoriales o beneficios tributarios
                        </div>
                    </div>
                    <div class="row-fluid form" style="text-align: left;" >
                        <div class="span2" >
                            <input type="checkbox" name="lista_elaboracion[]" id="elaboracion_ninguno" value="ninguno">Ninguno
                        </div>
                        <div class="span2" >
                            <input type="checkbox" name="lista_elaboracion[]" id="elaboracion_zonafranca" value="zonafranca">Zona Franca
                        </div>
                        <div class="span2" >
                            <input type="checkbox" name="lista_elaboracion[]" id="elaboracion_cedeims" value="cedeims">CEDEIMS
                        </div>
                        <div class="span2" >
                            <input type="checkbox" name="lista_elaboracion[]" id="elaboracion_ritex" value="ritex">RITEX
                        </div>
                        <div class="span4" style="display: inline;" >
                            <input type="checkbox" name="lista_elaboracion[]" id="elaboracion_otros" value="otros">Otros
                            <div style="display:none; text-align: left;" id="div_elaboraciondetalle">
                                <input type="text" class="k-textbox"  placeholder="Detallar"  name="elaboracion_detalle" id="elaboracion_detalle" /> 
                            </div>
                        </div>
                    </div>
                    <div class="row-fluid" >
                            <div class="span" ><center>
                                 <input type="hidden" name="hiddenvalidatorelaboracion" id="hiddenvalidatorelaboracion" 
                         data-elaboracionvalidator
                         data-required-msg="Escoja por lo menos una opción" /></center>
                            </div> 
                    </div>
                    <div class="row-fluid form">
                        <div class="barra">
                        </div>
                    </div>
                    
                    <div class="row-fluid form" >
                        <div class="span12" >
                             <input type="hidden" name="hiddenvalidatorgrid" id="hiddenvalidatorgrid" 
                             data-gridvalidator
                             data-required-msg="Por Favor Llene por lo menos un dato de Insumos Nacionales o Importados" />
                        </div> 
                    </div>
                    <!-- BOTONES -->
                    <div class="row-fluid form" >
                        <div class="span4">
                        <input id="pedirasesoramiento" type="button" value="Pedir Asesoramiento" class="k-primary" style="width:100%"/> <br><br>
                        </div>
                        <div class="span4">
                        <input id="guardarddjj" type="button" value="Declarar" class="k-primary" style="width:100%"/> <br><br>
                        </div>
                        <div class="span4">
                        <input id="cancelarddjj" type="button"  value="Cancelar" class="k-primary" style="width:100%"/> 
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>      
</div>
</form>

<div id="fabricas">
    <form name="form_fabricas" id="form_fabricas" method="post" action="index.php">
        <div class="titulo">Agregar Fábrica</div><br>
        
        <div class="row-fluid form">
            <input type="text" class="k-textbox" id="ciudad_fabrica" name="ciudad_fabrica" style="width:100%;" placeholder="Ciudad donde está la fábrica" required validationMessage="Ingrese la ciudad donde se encuentra la Fábrica">
            <br><br>
            <input type="text" class="k-textbox" id="direccion_fabrica" name="direccion_fabrica" style="width:100%;" placeholder="Dirección de la Fábrica" required validationMessage="Ingrese la dirección donde se encuentra la Fábrica">
            <br><br>
            <input type="text" class="k-textbox" id="telefono_fabrica" name="telefono_fabrica" style="width:100%;" placeholder="Número de Teléfono de contacto o celular" required validationMessage="Ingrese el Número de Teléfono de contacto o Celular">
            <br><br>
            <input type="text" class="k-textbox" id="persona_contacto" name="persona_contacto" style="width:100%;" placeholder="Persona de Contacto" required validationMessage="Ingrese el Número de Teléfono de contacto o Celular">
            <br>
        </div>
        <div class="row-fluid form">
            <input id="agregarfabrica" type="button"  value="Enviar" class="k-primary" style="width:40"/> 
            <input id="cancelarfabrica" type="button"  value="Cancelar" class="k-primary" style="width:40"/> 
        </div>
    </form>
</div> 

<div id="asesoramiento">
    <form name="envio_asesoramiento" id="envio_asesoramiento" method="post" action="index.php">
        <div class="titulo">Asesoramiento</div><br>
            <div id="mensajeasesoramiento">
            <p>Mediante esta herramienta le ayudaremos a obtener asesoramiento para el correcto<br>
               llenado de su Declaración Jurada. En qué desea que le ayudemos?
            </p>
            <input type="checkbox" name="asesorarddjj[]" id="check_arancel" value="arancel"> Clasificación Arancelaria.<BR>
            <input type="checkbox" name="asesorarddjj[]" id="check_procesoproductivo" value="procesoproductivo"> Proceso Productivo.<BR>
            <input type="checkbox" name="asesorarddjj[]" id="check_insumos" value="insumos"> Insumos Nacionales/Importados.<BR>
            <div class="span" ><center>
                <input type="hidden" name="hiddenvalidatorasesorar" id="hiddenvalidatorasesorar" 
                data-asesorarvalidator
                data-required-msg="Escoja escoger por lo menos una opción" /></center>
            </div>
            <br><br>
            <p>Realiza una pregunta más específica</p>
                <textarea class="k-textbox" rows="3" cols="47" id="observaciones" name="observaciones" placeholder="Observaciones"></textarea>
                <div class="span" ><center>
                    <input type="hidden" name="hiddenvalidatorobserv" id="hiddenvalidatorobserv" 
                    data-observvalidator
                    data-required-msg="Escoja escoger por lo menos una opción" /></center>
                </div>
                
                <br><br><br>
            <input id="asesoramientoddjj" type="button" value="Enviar" class="k-primary" /> 
            <input id="cancelarasesoramiento" type="button" value="Cancelar" class="k-primary" /> 
        </div>
    </form>
</div> 

<div class="row-fluid fadein"  id="firmaenvioddjjcompleta" >
       <div class="k-block fadein">
                <div class="k-header">
                    <div class="k-header"><div class="titulo">Confirmación de Envío de DD.JJ.</div></div>
                </div>
                <div class="k-cuerpo"> 
                    <div class="row-fluid  form" >
                        <div class="span1" > </div>
                        <div class="span10" >
                            <p> Yo <span class="letrarelevante">{$nombrecompleto}</span>, certifico el envío para revisión
                                de una declaración jurada.</p> 
                        </div>  
                        <div class="span1" ></div>
                    </div> 
                    <form name="formfirmaenvioddjjcompleta" id="formfirmaenvioddjjcompleta" method="post"  action="index.php" >
                        <div class="row-fluid  form" >
                            <div class="span5" ></div>
                            <div class="span2" >
                                <input type="text" onkeypress='return event.charCode >= 48 && event.charCode <= 57'
                               class="k-textbox " placeholder="Pin" name="pinfirmaenvioddjjcompleta"  id="pinfirmaenvioddjjcompleta" maxlength="4" size="4" style="width:50px;"
                               required data-required-msg="Por favor ingrese su Pin." data-firmarenvioddjjcompleta /> 
                            </div>  
                            <div class="span5" ></div>
                        </div>  
                        <div class="row-fluid  form" >
                            <div class="span4 hidden-phone" >
                             </div>
                             <div class="span2 " >
                                 <input id="cancelafirmaenvioddjjcompleta" type="button" value="Cancelar" class="k-primary" style="width:100%"/> <br> <br>
                             </div> 
                             <div class="span2 " >
                                 <input id="firmarenvioddjjcompleta" type="button"  value="Firmar" class="k-primary" style="width:100%"/>
                             </div> 
                             <div class="span4 hidden-phone" >
                             </div>
                         </div> 
                    </form> 
                </div>   
      </div>              
</div>

<div class="row-fluid fadein"  id="firmaenvioddjjasesoramiento" >
       <div class="k-block fadein">
                <div class="k-header">
                    <div class="k-header"><div class="titulo">Confirmación de Envío de DD.JJ.</div></div>
                </div>
                <div class="k-cuerpo"> 
                    <div class="row-fluid  form" >
                        <div class="span1" > </div>
                        <div class="span10" >
                            <p> Yo <span class="letrarelevante">{$nombrecompleto}</span>, certifico el envío para revisión
                                de una declaración jurada con Asesoramiento.</p> 
                        </div>  
                        <div class="span1" ></div>
                    </div> 
                    <form name="formfirmaenvioddjjasesoramiento" id="formfirmaenvioddjjasesoramiento" method="post"  action="index.php" >
                        <div class="row-fluid  form" >
                            <div class="span5" ></div>
                            <div class="span2" >
                                <input type="text" onkeypress='return event.charCode >= 48 && event.charCode <= 57'
                               class="k-textbox " placeholder="Pin" name="pinfirmaenvioddjjasesoramiento"  id="pinfirmaenvioddjjasesoramiento" maxlength="4" size="4" style="width:50px;"
                               required data-required-msg="Por favor ingrese su Pin." data-firmarenvioddjjasesoramiento /> 
                            </div>  
                            <div class="span5" ></div>
                        </div>  
                        <div class="row-fluid  form" >
                            <div class="span4 hidden-phone" >
                             </div>
                             <div class="span2 " >
                                 <input id="cancelafirmaenvioddjjasesoramiento" type="button" value="Cancelar" class="k-primary" style="width:100%"/> <br> <br>
                             </div> 
                             <div class="span2 " >
                                 <input id="firmarenvioddjjasesoramiento" type="button"  value="Firmar" class="k-primary" style="width:100%"/>
                             </div> 
                             <div class="span4 hidden-phone" >
                             </div>
                         </div> 
                    </form> 
                </div>   
      </div>              
</div>

<script>
$('input[type="text"],textarea').keypress(function(e){
    var regex = new RegExp("^[a-zA-Z0-9_, \b]+$");
    var str = String.fromCharCode(!e.charCode ? e.which : e.charCode);
    if (regex.test(str)) {
        return true;
    }
    e.preventDefault();
    return false;
});

    ocultar('firmaenvioddjjcompleta');
    ocultar('firmaenvioddjjasesoramiento');
    $("#pedirasesoramiento").kendoButton();
    $("#guardarddjj").kendoButton();
    $("#cancelarddjj").kendoButton();
    $("#asesoramientoddjj").kendoButton();
    $("#cancelarasesoramiento").kendoButton();
    $("#addfilainsumosimportados").kendoButton();
    $("#elimfilainsumosimportados").kendoButton();
    $("#addfilainsumosnacionales").kendoButton();
    $("#elimfilainsumosnacionales").kendoButton();
    $("#addfilacomercializador").kendoButton();
    $("#elimfilacomercializador").kendoButton();
    $("#addfabrica").kendoButton();
    $("#agregarfabrica").kendoButton();
    $("#cancelarfabrica").kendoButton();
    
    var pedirasesoramiento = $("#pedirasesoramiento").data("kendoButton");
    var guardarddjj = $("#guardarddjj").data("kendoButton");
    var cancelarddjj = $("#cancelarddjj").data("kendoButton");
    var asesoramientoddjj = $("#asesoramientoddjj").data("kendoButton");
    var cancelarasesoramiento = $("#cancelarasesoramiento").data("kendoButton");
    var addfabrica = $("#addfabrica").data("kendoButton");
    var agregarfabrica = $("#agregarfabrica").data("kendoButton");
    var cancelarfabrica = $("#cancelarfabrica").data("kendoButton");

    var fabricas = $("#fabricas").kendoWindow({
          draggable: true,
          height: "420px",                      
          width: "410px",
          resizable: true,
          modal: true,
          actions: [
                    "Close"
                ],
          animation: {
            close: {
              effects: "fade:out",
              duration: 1000
            }
            }
    }).data("kendoWindow");
    fabricas.center(); 

    var asesoramiento = $("#asesoramiento").kendoWindow({
        draggable: true,
        height: "480px",                      
        width: "550px",
        resizable: true,
        modal: true,
        actions: [
                "Close"
            ],
        animation: {
            close: {
                effects: "fade:out",
                duration: 1000
            }
        }
    }).data("kendoWindow");
    asesoramiento.center(); 
    
    var aparecefob = 0;
    function aparecer_fob(fob){
        if($("#"+fob.id).is(':checked')) {
            aparecefob++;
            if(aparecefob>0){
                $('#div_fob').fadeIn(1000);
            }
        } else {
            aparecefob--;
            if(aparecefob<=0){
                $('#div_fob').fadeOut(1000);
                $('#valor_fob').val();
            }
        }
    }
    
    var apareceexwork = 0;
    function aparecer_exwork(exwork){
        if($("#"+exwork.id).is(':checked')) {
            apareceexwork++;
            if(apareceexwork>0){
                $('#div_exwork').fadeIn(1000);
            }
        } else {
            apareceexwork--;
            if(apareceexwork<=0){
                $('#div_exwork').fadeOut(1000);
                $('#valor_exwork').val();
            }
        }
    }
    
    var apareceNaladisa2007 = 0;
    function aparecer_Naladisa2007(Naladisa2007){
        if($("#"+Naladisa2007.id).is(':checked')) {
            apareceNaladisa2007++;
            if(apareceNaladisa2007>0){
                $('#div_naladisa2007').fadeIn(1000);
            }
        } else {
            apareceNaladisa2007--;
            if(apareceNaladisa2007<=0){
                $('#div_naladisa2007').fadeOut(1000);
                $('#naladisa2007').val('');
            }
        }
    }
    
    var contarAcuerdos = 0;
    function cuentaAcuerdos(acuerdo){
        if($("#"+acuerdo.id).is(':checked')) {
            contarAcuerdos++;
            if(contarAcuerdos<=3){
                $('#total_ddjj').val('3');
            }else{
                var total_ddjj = $('#total_ddjj').val();
                total_ddjj=parseFloat(total_ddjj) + (0.5);
                $('#total_ddjj').val(total_ddjj);
            }
        } else {
            contarAcuerdos--;
            if(contarAcuerdos<=3){
                if(contarAcuerdos==0){
                    $('#total_ddjj').val('0');
                }else{
                    $('#total_ddjj').val('3');
                }
            }else{
                var total_ddjj = $('#total_ddjj').val();
                total_ddjj=parseFloat(total_ddjj) - (0.5);
                $('#total_ddjj').val(total_ddjj);
            }
        }
    }
    
    function quitarAcuerdosSgp(){
        contarAcuerdos--;
        if(contarAcuerdos<=3){
            if(contarAcuerdos==0){
                $('#total_ddjj').val('0');
            }else{
                $('#total_ddjj').val('3');
            }
        }else{
            var total_ddjj = $('#total_ddjj').val();
            total_ddjj=parseFloat(total_ddjj) - (0.5);
            $('#total_ddjj').val(total_ddjj);
        }
    }
    
    ///FUNCIONES PARA LA VENTANA DE FÁBRICA
    addfabrica.bind("click", function(e){
        fabricas.open();
    });
    
    agregarfabrica.bind("click", function(e){
        if(form_fabricas.validate()){
            var datos = $("#form_fabricas").serialize()+"&opcion=admEmpresa&tarea=guardarFabrica";
            $.ajax({
                type: 'post',
                url: 'index.php',
                data: datos,
                success: function (data) {
                    if(data!=0){
                        var fabrica = $("#ciudad_fabrica").val()+' - '+$("#direccion_fabrica").val() 
                        fabricas.close();
                        var combobox = $("#combo_fabricas").data("kendoComboBox");
                        {literal}
                        combobox.dataSource.add({"fabricas": fabrica,"id_fabrica":data});
                        {/literal}
                        $("#form_fabricas")[0].reset();
                    }
                }
            }); 
        }
    });
    
    cancelarfabrica.bind("click", function(e){
        $("#form_fabricas")[0].reset();
        fabricas.close();
    });
    
    var form_fabricas = $("#form_fabricas").kendoValidator({
    }).data("kendoValidator");
    
    //FUNCIONES PARA LA FIRMA DEL EXPORTADOR ENVIANDO LA DDJJ COMPLETA
    $("#cancelafirmaenvioddjjcompleta").kendoButton();
    $("#firmarenvioddjjcompleta").kendoButton();
    var cancelafirmaenvioddjjcompleta = $("#cancelafirmaenvioddjjcompleta").data("kendoButton");
    var firmarenvioddjjcompleta = $("#firmarenvioddjjcompleta").data("kendoButton");
    var datos;

    cancelafirmaenvioddjjcompleta.bind("click", function(e){             
        cambiar('firmaenvioddjjcompleta','enviardeclaracionjurada');
        borrarPin('{$nombre}');
    });
    
    firmarenvioddjjcompleta.bind("click", function(e){
        if(formfirmaenvioddjjcompleta.validate())
        {
            //Capturar los datos del formulario y los insumos nacionales, enviar controlador y tarea
            //almacenar todos los insumos nacionales
            var grid_nac = $("#tabla_insumosnacionales").data("kendoGrid");
            var data_nac = grid_nac.dataSource;
            var numfilas_nac = data_nac.total();
            var valores_nac = data_nac.data();

            var arr_nac = [];
            for (var i = 0; i < numfilas_nac; i++) {
                var valores=valores_nac[i].descripcion+";"+valores_nac[i].fabricante+";"+valores_nac[i].telefono+";"+valores_nac[i].valor;
                arr_nac.push(valores);
            }
        
            var datos = $("#alta_ddjj").serialize()+"&opcion=admDeclaracionJurada&tarea=guardarDeclaracionJurada&tabla_nac="+arr_nac;

            //Verificar si hay Comercializadores
            var check_comerc = $('#check_comercializador').prop('checked');
            if(check_comerc){
                var grid_comerc = $("#tabla_comercializadores").data("kendoGrid");
                var data_comerc = grid_comerc.dataSource;
                var numfilas_comerc = data_comerc.total();
                var valores_comerc = data_comerc.data();

                var arr_comerc = [];
                for (var i = 0; i < numfilas_comerc; i++) {
                    var valores=valores_comerc[i].razon_social+";"+valores_comerc[i].ci_nit+";"+valores_comerc[i].domicilio+";"+valores_comerc[i].representante_legal+";"+valores_comerc[i].direccion_fabrica+";"+valores_comerc[i].telefono+";"+valores_comerc[i].precio_venta+";"+valores_comerc[i].unidad_medida+";"+valores_comerc[i].produccion_mensual;
                    arr_comerc.push(valores);
                }
                datos=datos+"&tabla_comerc="+arr_comerc;
            }

            //Verificar si hay Insumos Importados
            var check_import = $('#check_insumosimportados').prop('checked');
            if(check_import){
                var grid_import = $("#tabla_insumosimportados").data("kendoGrid");
                var data_import = grid_import.dataSource;
                var numfilas_import = data_import.total();
                var valores_import = data_import.data();

                var arr_import = [];
                for (var i = 0; i < numfilas_import; i++) {
                    var valores=valores_import[i].descripcion+";"+valores_import[i].nandina+";"+valores_import[i].pais+";"+valores_import[i].productor+";"+valores_import[i].cuenta_co+";"+valores_import[i].acuerdo+";"+valores_import[i].cantidad+";"+valores_import[i].unidad_medida+";"+valores_import[i].valor;
                    arr_import.push(valores);
                }
                datos=datos+"&tabla_import="+arr_import;
            }
            
            $.ajax({
                type: 'post',
                url: 'index.php',
                data: datos,
                success: function(data) {
                    firmarPin('Declaración Jurada','admDeclaracionJurada','','{$nombre}');
                }
            });
        }
    });

    /*-----------esto es para la validacion del pin*****************************************/
    var swfirma=2;        
    var firmacache='';
    var formfirmaenvioddjjcompleta = $("#formfirmaenvioddjjcompleta").kendoValidator({
        rules:{ 
            firmarenvioddjjcompleta: function(input) { 
                var validate = input.data('firmarenvioddjjcompleta');
                if (typeof validate !== 'undefined' && validate !== false) 
                {
                    if (firmacache !== $("#pinfirmaenvioddjjcompleta").val()) 
                    {
                        verificarPinDDJJ($("#pinfirmaenvioddjjcompleta").val());                    
                        return false;
                    }
                    if(swfirma==1)
                    {
                         return true;
                    }
                    if(swfirma==0)
                    {  
                        return false;
                    }  
                    return false;
                }

               return true;
            }
        },
        messages:{
            firmarenvioddjjcompleta: function(input) { 
                if(swfirma==0)
                {  
                    return 'El Pin no es Correcto';
                }
                else
                {    
                    return 'Revisando..';
                }
            }
       }
    }).data("kendoValidator");
    
    function verificarPinDDJJ(pin_insertado)
    {
        $.ajax({
                type: 'post',
                url: 'index.php',
                data: 'opcion=funcionesGenerales&tarea=verificarPin&id_transaccion_pin='+id_transaccion_pin+'&pin_introducido='+pin_insertado+'',
                success: function (data) {    
                    swfirma=data;
                    firmacache=$("#pinfirmaenvioddjjcompleta").val();
                    formfirmaenvioddjjcompleta.validateInput($("#pinfirmaenvioddjjcompleta"));
                }
            }); 
    }


    //FUNCIONES PARA LA FIRMA DEL EXPORTADOR ENVIANDO LA DDJJ CON ASESORAMIENTO
    $("#cancelafirmaenvioddjjasesoramiento").kendoButton();
    $("#firmarenvioddjjasesoramiento").kendoButton();
    var cancelafirmaenvioddjjasesoramiento = $("#cancelafirmaenvioddjjasesoramiento").data("kendoButton");
    var firmarenvioddjjasesoramiento = $("#firmarenvioddjjasesoramiento").data("kendoButton");

    cancelafirmaenvioddjjasesoramiento.bind("click", function(e){             
        cambiar('firmaenvioddjjasesoramiento','enviardeclaracionjurada');
        borrarPin('{$nombre}');
    });
    
    firmarenvioddjjasesoramiento.bind("click", function(e){
        if(formfirmaenvioddjjasesoramiento.validate())
        {
            var datos = $("#alta_ddjj").serialize()+"&opcion=admDeclaracionJurada&tarea=pedirAsesoramiento&"+$("#envio_asesoramiento").serialize();

            $.ajax({
                type: 'post',
                url: 'index.php',
                data: datos,
                success: function(data) {
                    firmarPin('Declaración Jurada','admDeclaracionJurada','','{$nombre}');
                }
            });
        }
    });

    /*-----------esto es para la validacion del pin*****************************************/
    var swfirma=2;        
    var firmacache='';
    var formfirmaenvioddjjasesoramiento = $("#formfirmaenvioddjjasesoramiento").kendoValidator({
        rules:{ 
            firmarenvioddjjasesoramiento: function(input) { 
                var validate = input.data('firmarenvioddjjasesoramiento');
                if (typeof validate !== 'undefined' && validate !== false) 
                {
                    if (firmacache !== $("#pinfirmaenvioddjjasesoramiento").val()) 
                    {
                        verificarPinDDJJasesoramiento($("#pinfirmaenvioddjjasesoramiento").val());                    
                        return false;
                    }
                    if(swfirma==1)
                    {
                         return true;
                    }
                    if(swfirma==0)
                    {  
                        return false;
                    }  
                    return false;
                }

               return true;
            }
        },
        messages:{
            firmarenvioddjjasesoramiento: function(input) { 
                if(swfirma==0)
                {  
                    return 'El Pin no es Correcto';
                }
                else
                {    
                    return 'Revisando..';
                }
            }
       }
    }).data("kendoValidator");
    function verificarPinDDJJasesoramiento(pin_insertado)
    {
        $.ajax({
                type: 'post',
                url: 'index.php',
                data: 'opcion=funcionesGenerales&tarea=verificarPin&id_transaccion_pin='+id_transaccion_pin+'&pin_introducido='+pin_insertado+'',
                success: function (data) {    
                    swfirma=data;
                    firmacache=$("#pinfirmaenvioddjjasesoramiento").val();
                    formfirmaenvioddjjasesoramiento.validateInput($("#pinfirmaenvioddjjasesoramiento"));
                }
            }); 
    }



    //GUARDAR LA DDJJ COMPLETA SIN ASESORAMIENTO
    var alta_ddjj = $("#alta_ddjj").kendoValidator({
        rules:{
            checkvalidator: function(input) { 
                var validate = input.data('checkvalidator');
                if (typeof validate !== 'undefined') 
                {
                    if(verificalistaacuerdos()){
                        return true;
                    }else{
                        return false;
                    }
                }
                return true;
            },
            elaboracionvalidator: function(input) { 
                var validate = input.data('elaboracionvalidator');
                if (typeof validate !== 'undefined') 
                {
                    if(verificalistaelaboracion()){
                        return true;
                    }else{
                        return false;
                    }
                }
                return true;
            },
            gridvalidator: function(input) {
                var validate = input.data('gridvalidator');
                if (typeof validate !== 'undefined') 
                {
                    //---aqui cuentas los productos y verificas si no estan vacíos
                    swp=verificagridinsumosnacionales();
                    if(swp==0)
                    {
                        return false;
                    }else if(swp==1)
                    {
                        return false;
                    }else if(swp==2)
                    {
                        return true
                    }else if(swp==3)
                    {
                        return false
                    }else if(swp==4)
                    {
                        return false
                    }else if(swp==5)
                    {
                        return false
                    }else if(swp==6)
                    {
                        return false
                    }
                }
                return true;
            },
            fobvalidator: function(input) {
                var validate = input.data('fobvalidator');
                if (typeof validate !== 'undefined') 
                {
                    //Verificar los Check de Acuerdos para Valor FOB
                    var check_can = $('#check_can').prop('checked');
                    var check_ace22 = $('#check_ace22').prop('checked');
                    var check_ace47 = $('#check_ace47').prop('checked');
                    var check_arpar4 = $('#check_arpar4').prop('checked');
                    var check_aapag = $('#check_aapag').prop('checked');
                    var check_mercosur = $('#check_mercosur').prop('checked');
                    var check_venezuela = $('#check_venezuela').prop('checked');
                    var check_arampanama = $('#check_arampanama').prop('checked');

                    if((check_can)||(check_ace22)||(check_ace47)||(check_arpar4)||(check_aapag)||(check_mercosur)||(check_venezuela)||(check_arampanama)){
                        var valor_fob = $('#valor_fob').val();
                        if(valor_fob==''){
                            return false;
                        }else{
                            return true;
                        }
                    }
                }
                return true;
            },
            exworkvalidator: function(input){
                var validate = input.data('exworkvalidator');
                if (typeof validate !== 'undefined') 
                {
                    //Verificar los Check de Acuerdos para Valor EX-WORK
                    var sgp_canada = $('#sgp_canada').prop('checked');
                    var sgp_suiza = $('#sgp_suiza').prop('checked');
                    var sgp_noruega = $('#sgp_noruega').prop('checked');
                    var sgp_japon = $('#sgp_japon').prop('checked');
                    var sgp_zelanda = $('#sgp_zelanda').prop('checked');
                    var sgp_rusia = $('#sgp_rusia').prop('checked');
                    var sgp_turquia = $('#sgp_turquia').prop('checked');
                    var sgp_bielorrusia = $('#sgp_bielorrusia').prop('checked');
                    var sgp_ue = $('#sgp_ue').prop('checked');
                    var sgp_eeuu = $('#sgp_eeuu').prop('checked');
                    var sgp_tp = $('#sgp_tp').prop('checked');

                    if((sgp_canada)||(sgp_suiza)||(sgp_noruega)||(sgp_japon)||(sgp_zelanda)||(sgp_rusia)||(sgp_turquia)||(sgp_bielorrusia)||(sgp_ue)||(sgp_eeuu)||(sgp_tp)){
                        var valor_exwork = $('#valor_exwork').val();
                        if(valor_exwork==''){
                            return false;
                        }else{
                            return true;
                        }
                    }
                }
                return true;
            },
            naladisa93validator: function(input) {
                var validate = input.data('naladisa93validator');
                if (typeof validate !== 'undefined') 
                {
                    //Verificar los Check de Acuerdos para NALADISA93
                    var check_mercosur = $('#check_mercosur').prop('checked');

                    if(check_mercosur){
                        var naladisa93 = $('#naladisa93').val();
                        if(naladisa93==''){
                            return false;
                        }else{
                            return true;
                        }
                    }
                }
                return true;
            },
            naladisa96validator: function(input) {
                var validate = input.data('naladisa96validator');
                if (typeof validate !== 'undefined') 
                {
                    //Verificar los Check de Acuerdos para NALADISA96
                    var check_mercosur = $('#check_ace22').prop('checked');

                    if(check_mercosur){
                        var naladisa96 = $('#naladisa96').val();
                        if(naladisa96==''){
                            return false;
                        }else{
                            return true;
                        }
                    }
                }
                return true;
            },
            naladisa2007validator: function(input) {
                var validate = input.data('naladisa2007validator');
                if (typeof validate !== 'undefined') 
                {
                    //Verificar los Check de Acuerdos para NALADISA2007
                    var check_arampanama = $('#check_arampanama').prop('checked');
                    var check_ace47 = $('#check_ace47').prop('checked');

                    if((check_arampanama)||(check_ace47)){
                        var naladisa2007 = $('#naladisa2007').val();
                        if(naladisa2007==''){
                            return false;
                        }else{
                            return true;
                        }
                    }
                }
                return true;
            },
            naladisa91validator: function(input) {
                var validate = input.data('naladisa91validator');
                if (typeof validate !== 'undefined') 
                {
                    //Verificar los Check de Acuerdos para NALADISA91
                    var check_aapag = $('#check_aapag').prop('checked');

                    if(check_aapag){
                        var naladisa91 = $('#naladisa91').val();
                        if(naladisa91==''){
                            return false;
                        }else{
                            return true;
                        }
                    }
                }
                return true;
            },
            /*
            naladi83validator: function(input) {
                var validate = input.data('naladi83validator');
                if (typeof validate !== 'undefined') 
                {
                    //Verificar los Check de Acuerdos para NALADI83
                    var check_arpar4 = $('#check_arpar4').prop('checked');

                    if(check_mercosur){
                        var naladi83 = $('#naladi83').val();
                        if(naladi83==''){
                            return false;
                        }else{
                            return true;
                        }
                    }
                }
                return true;
            },*/
        },
        messages:{
            checkvalidator: "Escoja al menos uno de los acuerdos",
            elaboracionvalidator: "Escoja al menos una opción",
            gridvalidator: function(input) { 
                if(swp==0)
                {  
                  return 'Ingrese al menos un Insumo Nacional o Importado';
                }
                else if(swp==1)
                {    
                  return 'Por favor Complete los datos';
                }else if(swp==3)
                {    
                  return 'Primero debe colocar el valor FOB de la mercancía';
                }else if(swp==4)
                {    
                  return 'Primero debe colocar el valor EXWORK de la mercancía';
                }else if(swp==5)
                {    
                  return 'El valor puesto en los insumos sobrepasa el valor FOB';
                }else if(swp==6)
                {    
                  return 'El valor puesto en los insumos sobrepasa el valor EXWORK';
                }else if(swp==7)
                {  
                  return 'Ingrese al menos un Insumo Importado o borre la fila';
                }
                
            },
            fobvalidator: "Ingrese el Valor FOB de la Mercancía",
            exworkvalidator: "Ingrese el Valor EX-Work de la Mercancía",
            naladisa93validator: "Ingrese el Valor NALADISA93 de la Mercancía",
            naladisa96validator: "Ingrese el Valor NALADISA96 de la Mercancía",
            naladisa2007validator: "Ingrese el Valor NALADISA2007 de la Mercancía",
            naladisa91validator: "Ingrese el Valor NALADISA91 de la Mercancía",
            //naladi83validator: "Ingrese el Valor NALADI83 de la Mercancía"
        }
    }).data("kendoValidator");

    function verificalistaacuerdos(){
        var sw=1;
        $("input:checkbox[name='lista_acuerdos[]']:checked").each(function(){
             sw=0;
        });
        if(sw==0){
            return true
        }else{
            return false
        }
    }
    
    function verificalistaelaboracion(){
        var sw=1;
        $("input:checkbox[name='lista_elaboracion[]']:checked").each(function(){
             sw=0;
        });
        if(sw==0){
            return true
        }else{
            return false
        }
    }
    
    function verificagridinsumosnacionales()
    {
        var total_nacionales = 0;
        var total_importados = 0;
        var existe_fob=0;
        var existe_exwork=0;
        //Verificar los Check de Acuerdos para Valor FOB
        var check_can = $('#check_can').prop('checked');
        var check_ace22 = $('#check_ace22').prop('checked');
        var check_ace47 = $('#check_ace47').prop('checked');
        var check_arpar4 = $('#check_arpar4').prop('checked');
        var check_aapag = $('#check_aapag').prop('checked');
        var check_mercosur = $('#check_mercosur').prop('checked');
        var check_venezuela = $('#check_venezuela').prop('checked');
        var check_arampanama = $('#check_arampanama').prop('checked');
        if((check_can)||(check_ace22)||(check_ace47)||(check_arpar4)||(check_aapag)||(check_mercosur)||(check_venezuela)||(check_arampanama)){
            if($('#valor_fob').val()==''){
                return 3;
            }else{
                existe_fob=1;
                var valor_fob = parseFloat($('#valor_fob').val());
            }
        }
        //Verificar los Check de Acuerdos para Valor EX-WORK
        var sgp_canada = $('#sgp_canada').prop('checked');
        var sgp_suiza = $('#sgp_suiza').prop('checked');
        var sgp_noruega = $('#sgp_noruega').prop('checked');
        var sgp_japon = $('#sgp_japon').prop('checked');
        var sgp_zelanda = $('#sgp_zelanda').prop('checked');
        var sgp_rusia = $('#sgp_rusia').prop('checked');
        var sgp_turquia = $('#sgp_turquia').prop('checked');
        var sgp_bielorrusia = $('#sgp_bielorrusia').prop('checked');
        var sgp_ue = $('#sgp_ue').prop('checked');
        var sgp_eeuu = $('#sgp_eeuu').prop('checked');
        var sgp_tp = $('#sgp_tp').prop('checked');
        if((sgp_canada)||(sgp_suiza)||(sgp_noruega)||(sgp_japon)||(sgp_zelanda)||(sgp_rusia)||(sgp_turquia)||(sgp_bielorrusia)||(sgp_ue)||(sgp_eeuu)||(sgp_tp)){
            if($('#valor_exwork').val()==''){
                return 4;
            }else{
                existe_exwork=1;
                var valor_exwork = parseFloat($('#valor_exwork').val());
            }
        }

        var grid_nac = $("#tabla_insumosnacionales").data("kendoGrid");
        var data_nac = grid_nac.dataSource;
        var numfilas_nac = data_nac.total();
        
        var grid_imp = $("#tabla_insumosimportados").data("kendoGrid");
        var data_imp = grid_imp.dataSource;
        var numfilas_imp = data_imp.total();
        
        if((numfilas_nac==0)&&(numfilas_imp==0)){
            return 0;
        }else{
            var valores_nac = data_nac.data();
            var arr_nac = [];
            for (var i = 0; i < numfilas_nac; i++) {
                total_nacionales = total_nacionales + valores_nac[i].valor;
                if(!valores_nac[i].descripcion.trim() || !valores_nac[i].fabricante.trim() || valores_nac[i].telefono=='0' || valores_nac[i].valor=='0')
                {
                    return 1;
                }              
            }
            if(existe_fob==1){
                if(total_nacionales >= valor_fob){
                    return 5;
                }
            }
            if((existe_exwork==1)&&(total_nacionales>=parseFloat(valor_exwork))){
                return 6;
            }
        }
        
        var importados = $('#check_insumosimportados').prop('checked');
        if(importados){
            var grid_imp = $("#tabla_insumosimportados").data("kendoGrid");
            var data_imp = grid_imp.dataSource;
            var numfilas_imp = data_imp.total();
            if(numfilas_imp!=0){
                var valores_imp = data_imp.data();
                var arr_imp = [];
                for (var i = 0; i < numfilas_imp; i++) {
                    total_importados = total_importados + valores_imp[i].valor;
                    if(!valores_imp[i].descripcion.trim() || !valores_imp[i].pais.trim() || !valores_imp[i].productor.trim() || !valores_imp[i].cuenta_co.trim() || !valores_imp[i].acuerdo.trim() || valores_imp[i].cantidad=='0' || !valores_imp[i].unidad_medida.trim() || valores_imp[i].valor=='0')
                    {
                        return 1;
                    } 
                }
                if(existe_fob==1){
                    if(total_nacionales >= valor_fob){
                        return 5;
                    }
                }
                if((existe_exwork==1)&&(total_importados>=parseFloat(valor_exwork))){
                    return 6;
                }  
            }
        }
        total = total_nacionales+total_importados;
        if(total>=valor_fob){
           return 5;
        }
        if(total>=valor_exwork){
           return 6; 
        }
        return 2;
    }
    
    guardarddjj.bind("click", function(e){
        if(alta_ddjj.validate()){
            cambiar('enviardeclaracionjurada','firmaenvioddjjcompleta');
            generarPin('{$id_empresa}','{$id_persona}','4');
        }
    });
    
    cancelarddjj.bind("click", function(e){  
        remover(tabStrip.select());
    });
    
    
    //****************** PROCESO PARA PEDIR ASESORAMIENTO EN LA DDJJ **********************
    var validator = $("#alta_ddjj").kendoValidator({
        rules:{
            checkvalidator: function(input) { 
                var validate = input.data('checkvalidator');
                if (typeof validate !== 'undefined' && validate !== false) {
                    if(verificalistaacuerdos())
                    {
                        return true;
                    }
                    else
                    {
                        return false;
                    }
                }
                return true;
            }
        },
        messages:{
            checkvalidator: "Escoja al menos uno de los acuerdos",
        }
    }).data("kendoValidator");

    pedirasesoramiento.bind("click", function(e){
        validator.validateInput($("#denominacion_comercial"));
        validator.validateInput($("#nombre_tecnico"));
        validator.validateInput($("#caracteristicas"));
        validator.validateInput($("#aplicacion"));
        validator.validateInput($("#hiddenvalidator"));
        if(validator.validateInput($("#denominacion_comercial")) && validator.validateInput($("#caracteristicas")) && validator.validateInput($("#hiddenvalidator")))
        {
            asesoramiento.open();
        }
    });

    cancelarasesoramiento.bind("click", function(e){
        $('#check_arancel').prop('checked','');
        $('#check_procesoproductivo').prop('checked','');
        $('#check_insumos').prop('checked','');
        $('#observaciones').val('');
        asesoramiento.close();
    });
    
    var envio_asesoramiento = $("#envio_asesoramiento").kendoValidator({
        rules:{
            asesorarvalidator: function(input) { 
                var validate = input.data('asesorarvalidator');
                if (typeof validate !== 'undefined')
                {
                    if(verificalistaasesoramiento())
                    {
                        return true;
                    }
                    else
                    {
                        return false;
                    }
                }
                return true;
            },
            observvalidator: function(input) { 
                var validate = input.data('observvalidator');
                if (typeof validate !== 'undefined') 
                {
                    var obs = $('#observaciones').val();
                    if(obs==''){
                        return false;
                    }else{
                        return true;
                    }
                }
                return true;
            },
        },
        messages:{
            asesorarvalidator: "Escoja escoger por lo menos una opción",
            observvalidator: "Debe describir su duda o pregunta claramente",
        }
    }).data("kendoValidator");
    
    function verificalistaasesoramiento(){
        var sw=1;
        $("input:checkbox[name='asesorarddjj[]']:checked").each(function(){
             sw=0;
        });
        if(sw==0){
            return true
        }else{
            return false
        }
    }
    
    asesoramientoddjj.bind("click", function(e){
        if(envio_asesoramiento.validate()){
            asesoramiento.close();
            cambiar('enviardeclaracionjurada','firmaenvioddjjasesoramiento');
            generarPin('{$id_empresa}','{$id_persona}','4');
            /*
            var datos = $("#alta_ddjj").serialize()+"&opcion=admDeclaracionJurada&tarea=pedirAsesoramiento&observaciones="+observaciones;

            $.ajax({
                type: 'post',
                url: 'index.php',
                data: datos,
                success: function(data) {
                    //alert("El Pedido de Asesoramiento fue enviado");
                    asesoramiento.close();
                    //remover(tabStrip.select());
                }
            });
    */
        }
    });
    
    //**************** MODELOS Y ESQUEMAS PARA LOS COMBOS Y LOS GRIDS *************//
    var unidadmedida = [
    {foreach $unidadmedida as $um} 
    {
        "id_unidad_medida": "{$um->getId_unidad_medida()}",
        "descripcion": "{$um->getDescripcion()}"
    },
    {/foreach} 
    ];
    var dsumedida = new kendo.data.DataSource({
            data: unidadmedida
    });
    function UMedidaDropDownEditor(container, options) {
        $('<input required data-text-field="descripcion" data-value-field="descripcion" data-bind="value:' + options.field + '"/>')
            .appendTo(container)
            .kendoDropDownList({
                    autoBind: false,
                    dataSource: dsumedida,
                    select: onSelectunidadmedida,
        });
    }

    function onSelectunidadmedida(e) {
        var dataItem = this.dataItem(e.item.index());
    };

    var dataComercializadores = new kendo.data.DataSource({
        schema: {
            model: {
                id: "id_comercializador",
                fields: {
                    razon_social: { 
                        defaultValue:"",
                         validation: {
                            required: true,
                            razon_socialvalidation: function (input) {
                                if (input.is("[name='razon_social']") && input.val() == " ") {
                                    input.attr("data-razon_socialvalidation-msg", "Debe ingresar una Razón Social");
                                    return /^[A-Z]/.test(input.val());
                                }
                                return true;
                            }
                        }
                    },
                    ci_nit: {
                        type: "number",
                        validation: {
                            required: true,
                            ci_nitvalidation: function (input) {
                                if (input.is("[name='ci_nit']") && input.val() == " ") {
                                    input.attr("data-ci_nitvalidation-msg", "Debe ingresar un CI o NIT");
                                    return /^[A-Z]/.test(input.val());
                                }
                                return true;
                            }
                        }
                    },
                    domicilio: { 
                        type: "string",
                        validation: {
                            required: true,
                            domiciliovalidation: function (input) {
                                if (input.is("[name='domicilio']") && input.val() == " ") {
                                    input.attr("data-domiciliovalidation-msg", "Debe ingresar una Dirección");
                                    return /^[A-Z]/.test(input.val());
                                }

                                return true;
                            }
                        }
                    },
                    representante_legal: {
                        type: "string",
                        validation: {
                            required: true,
                            representante_legalvalidation: function (input) {
                                if (input.is("[name='representante_legal']") && input.val() == " ") {
                                    input.attr("data-representante_legalvalidation-msg", "Debe ingresar el Representante Legal");
                                    return /^[A-Z]/.test(input.val());
                                }

                                return true;
                            }
                        }
                    },
                    direccion_fabrica: { 
                        type: "string",
                        validation: {
                            required: true,
                            direccion_fabricavalidation: function (input) {
                                if (input.is("[name='direccion_fabrica']") && input.val() == " ") {
                                    input.attr("data-direccion_fabricavalidation-msg", "Debe ingresar la Dirección");
                                    return /^[A-Z]/.test(input.val());
                                }

                                return true;
                            }
                        }
                    },
                    telefono: { 
                        type: "number",
                        validation: {
                            required: true,
                            telefonovalidation: function (input) {
                                if (input.is("[name='telefono']") && input.val() == "0") {
                                    input.attr("data-telefonovalidation-msg", "Debe ingresar un Número de Contacto");
                                    return /^[A-Z]/.test(input.val());
                                }

                                return true;
                            }
                        }
                    },
                    precio_venta: { 
                        type: "number",
                        validation: {
                            required: true,
                            precio_ventavalidation: function (input) {
                                if (input.is("[name='precio_venta']") && input.val() == "0") {
                                    input.attr("data-precio_ventavalidation-msg", "Debe ingresar el Precio de Venta al Comercializador");
                                    return /^[A-Z]/.test(input.val());
                                }

                                return true;
                            }
                        }
                    },
                    unidad_medida: {
                        type: "string"
                    },
                    produccion_mensual: { 
                        type: "number",
                        validation: {
                            required: true,
                            produccion_mensualvalidation: function (input) {
                                if (input.is("[name='produccion_mensual']") && input.val() == "0") {
                                    input.attr("data-produccion_mensualvalidation-msg", "Debe ingresar su Capacidad de Producción Mensual");
                                    return /^[A-Z]/.test(input.val());
                                }

                                return true;
                            }
                        }
                    }
                }
            }
        }
    });

    var pais = [
    {foreach $pais as $pa} 
    {
        "id_pais": "{$pa->getId_pais()}",
        "nombre": "{$pa->getNombre()}"
    },
    {/foreach} 
    ];

    var dspais = new kendo.data.DataSource({
            data: pais
    });

    function PaisDropDownEditor(container, options) {
        $('<input required data-text-field="nombre" data-value-field="nombre" data-bind="value:' + options.field + '"/>')
            .appendTo(container)
            .kendoDropDownList({
                    autoBind: false,
                    dataSource: dspais,
                    select: onSelectunidadmedida,
        });
    };

    function onSelectunidadmedida(e) {
        var dataItem = this.dataItem(e.item.index());
    };
    
    var acuerdo = [
    {foreach $acuerdo as $ac} 
    {
        "id_acuerdo": "{$ac->getId_Acuerdo()}",
        "descripcion": "{$ac->getDescripcion()}"
    },
    {/foreach} 
    ];
    var dsacuerdo = new kendo.data.DataSource({
            data: acuerdo
    });
    function AcuerdoDropDownEditor(container, options) {
        $('<input required data-text-field="descripcion" data-value-field="descripcion" data-bind="value:' + options.field + '"/>')
            .appendTo(container)
            .kendoDropDownList({
                    autoBind: false,
                    dataSource: dsacuerdo,
                    select: onSelectunidadmedida,
        });
    };

    function onSelectunidadmedida(e) {
        var dataItem = this.dataItem(e.item.index());
    };
    
    var tieneco = [
        { "valor": "0", "descripcion": "NO" },
        { "valor": "1", "descripcion": "SI" },
    ];
    var dstieneco = new kendo.data.DataSource({
            data: tieneco
    });
    function TienecoDropDownEditor(container, options) {
        $('<input data-bind="value:' + options.field + '"/>')
            .appendTo(container)
            .kendoDropDownList({
                    autoBind: true,
                    dataTextField: "descripcion",
                    dataValueField: "descripcion",
                    dataSource: dstieneco,
                    select: onSelectunidadmedida,
        });
    };
    
    var dataInsumosImportados = new kendo.data.DataSource({
        schema: {
            model: {
                id: "id_comercializador",
                fields: {
                    descripcion: { 
                        defaultValue:"",
                        validation: {
                            required: true,
                            descripcionvalidation: function (input) {
                                if (input.is("[name='descripcion']") && input.val() == " ") {
                                    input.attr("data-descripcionvalidation-msg", "Debe ingresar la Descripción");
                                    return /^[A-Z]/.test(input.val());
                                }
                                return true;
                            }
                        }
                    },
                    nandina: {
                        type: "number"
                    },
                    pais: { 
                        defaultValue:"NINGUNO",
                        type: "string"
                    },
                    productor: {
                        type: "string",
                        validation: {
                            required: true,
                            productorvalidation: function (input) {
                                if (input.is("[name='productor']") && input.val() == " ") {
                                    input.attr("data-productorvalidation-msg", "Debe ingresar un productor");
                                    return /^[A-Z]/.test(input.val());
                                }
                                return true;
                            }
                        }
                    },
                    cuenta_co: {
                        defaultValue:"NO",
                        type: "string"
                    },
                    acuerdo: {
                        defaultValue:"NINGUNO",
                        type: "string",
                    },
                    cantidad: { 
                        type: "number",
                        validation: {
                            required: true,
                            cantidadvalidation: function (input) {
                                if (input.is("[name='cantidad']") && input.val() == "0") {
                                    input.attr("data-cantidadvalidation-msg", "Debe ingresar la Cantidad");
                                    return /^[A-Z]/.test(input.val());
                                }
                                return true;
                            }
                        }
                    },
                    unidad_medida: {
                        type: "string"
                    },
                    valor: {
                        type: "number",
                        validation: {
                            required: true,
                            valorfobvalidation: function (input) {
                                if (input.is("[name='valorfob']") && input.val() == "0") {
                                    input.attr("data-valorfobvalidation-msg", "Debe ingresar el Valor");
                                    return /^[A-Z]/.test(input.val());
                                }
                                return true;
                            }
                        }
                    },
                }
            }
        }
    });

    {literal}
    function validatePass(e) {
        var regex = new RegExp("^[a-zA-Z0-9_, \b]+$");
        var str = String.fromCharCode(!e.charCode ? e.which : e.charCode);
        if (regex.test(str)) {
            return true;
        }
    }
    {/literal}
    var dataInsumosNacionales = new kendo.data.DataSource({
        schema: {
            model: {
                id: "id_insumos_nacionales",
                fields: {
                    descripcion: {
                        type: "string",
                        defaultValue:"",
                        validation: {
                            required: true,
                            descripcionvalidation: function (input) {
                                validatePass(input);
                                if (input.is("[name='descripcion']") && input.val() == " ") {
                                    input.attr("data-descripcionvalidation-msg", "Debe ingresar la Descripción");
                                    return /^[a-zA-Z0-9_, \b]/.test(input.val());
                                }
                                if(input.val()!="^[a-zA-Z0-9]+$"){
                                    input.attr("data-descripcionvalidation-msg", "Debe ingresar caracteres vialidos");
                                    return /^[a-zA-Z]/.test(input.val());
                                }
                                return true;
                            }
                        }
                    },
                    fabricante: { 
                        defaultValue:"",
                        validation: {
                            required: true,
                            fabricantevalidation: function (input) {
                                if (input.is("[name='fabricante']") && input.val() == " ") {
                                    input.attr("data-fabricantevalidation-msg", "Debe ingresar el Fabricante del Insumo");
                                    return /^[A-Z]/.test(input.val());
                                }
                                return true;
                            }
                        }
                    },
                    telefono: {
                        type: "number",
                        validation: {
                            required: true,
                            min: 0,
                            telefonovalidation: function (input) {
                                if (input.is("[name='telefono']") && input.val() == "0") {
                                    input.attr("data-telefonovalidation-msg", "Debe ingresar un Número de Contacto");
                                    return /^[A-Z]/.test(input.val());
                                }
                                return true;
                            }
                        }
                    },
                    valor: {
                        type: "number",
                        validation: {
                            required: true,
                            min: 0,
                            valorfobvalidation: function (input) {
                                if (input.is("[name='valorfob']") && input.val() == "0") {
                                    input.attr("data-valorfobvalidation-msg", "Debe ingresar el Valor");
                                    return /^[A-Z]/.test(input.val());
                                }
                                return true;
                            }
                        }
                    }
                }
            }
        }
    });
    
    $(document).ready(function(){
        $('#check_ace22').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                $('#div_naladisa96').fadeIn(1000);
            } else {
                $('#div_naladisa96').fadeOut(1000);
            }  
        });
        $('#check_arpar4').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                $('#div_naladi83').fadeIn(1000);
            } else {
                $('#div_naladi83').fadeOut(1000);
            }  
        });
        $('#check_aapag').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                $('#div_naladisa91').fadeIn(1000);
            } else {
                $('#div_naladisa91').fadeOut(1000);
            }  
        });
        $('#check_can').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                $('#div_nandina10').fadeIn(1000);
                $('#div_pais_can').fadeIn(1000);
            } else {
                $('#div_nandina10').fadeOut(1000);
                $('#div_pais_can').fadeOut(1000);
            }  
        });
        $('#check_mercosur').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                $('#div_naladisa93').fadeIn(1000);
            } else {
                $('#div_naladisa93').fadeOut(1000);
            }  
        });
        
        $('#check_comercializador').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                $('#div_comercializador').fadeIn(1000);
            } else {
                $('#div_comercializador').fadeOut(1000);
            }  
        });

        $('#check_insumosimportados').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                $('#div_insumosimportados').fadeIn(1000);
            } else {
                $('#div_insumosimportados').fadeOut(1000);
            }  
        });
        
        $('#check_sgt').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                $('#div_sgt').fadeIn(1000);
            } else {  
                $('#div_sgt').fadeOut(1000);
                $('#valor_exwork').val();
                //$('#div_exwork').fadeOut(1000);
                
                var marcado_canada = $("#sgp_canada").prop('checked');
                if(marcado_canada){
                    quitarAcuerdosSgp();
                    /*
                    var total=$('#total_ddjj').val();
                    var restar=parseFloat(total)-60;
                    $('#total_ddjj').val(restar);*/
                }
                
                var marcado_suiza = $("#sgp_suiza").prop('checked');
                if(marcado_suiza){
                    quitarAcuerdosSgp();
                    /*
                    var total=$('#total_ddjj').val();
                    var restar=parseFloat(total)-60;
                    $('#total_ddjj').val(restar);
                    */
                }
            
                var marcado_noruega = $("#sgp_noruega").prop('checked');
                if(marcado_noruega){
                    quitarAcuerdosSgp();
                    /*
                    var total=$('#total_ddjj').val();
                    var restar=parseFloat(total)-60;
                    $('#total_ddjj').val(restar);
                    */
                }
                var marcado_japon= $("#sgp_japon").prop('checked');
                if(marcado_japon){
                    quitarAcuerdosSgp();
                    /*
                    var total=$('#total_ddjj').val();
                    var restar=parseFloat(total)-60;
                    $('#total_ddjj').val(restar);
                    */
                }
                var marcado_zelanda = $("#sgp_zelanda").prop('checked');
                if(marcado_zelanda){
                    quitarAcuerdosSgp();
                    /*
                    var total=$('#total_ddjj').val();
                    var restar=parseFloat(total)-60;
                    $('#total_ddjj').val(restar);
                    */
                }
                var marcado_rusia = $("#sgp_rusia").prop('checked');
                if(marcado_rusia){
                    quitarAcuerdosSgp();
                    /*
                    var total=$('#total_ddjj').val();
                    var restar=parseFloat(total)-60;
                    $('#total_ddjj').val(restar);
                    */
                }
                var marcado_turquia= $("#sgp_turquia").prop('checked');
                if(marcado_turquia){
                    quitarAcuerdosSgp();
                    /*
                    var total=$('#total_ddjj').val();
                    var restar=parseFloat(total)-60;
                    $('#total_ddjj').val(restar);*/
                }
                var marcado_bielorrusia = $("#sgp_bielorrusia").prop('checked');
                if(marcado_bielorrusia){
                    quitarAcuerdosSgp();
                    /*
                    var total=$('#total_ddjj').val();
                    var restar=parseFloat(total)-60;
                    $('#total_ddjj').val(restar);*/
                }
                var marcado_ue = $("#sgp_ue").prop('checked');
                if(marcado_ue){
                    quitarAcuerdosSgp();
                    /*
                    var total=$('#total_ddjj').val();
                    var restar=parseFloat(total)-60;
                    $('#total_ddjj').val(restar);*/
                }
                var marcado_eeuu = $("#sgp_eeuu").prop('checked');
                if(marcado_eeuu){
                    quitarAcuerdosSgp();
                    /*
                    var total=$('#total_ddjj').val();
                    var restar=parseFloat(total)-60;
                    $('#total_ddjj').val(restar);*/
                }
                
                $('#sgp_japon').prop('checked','');
                $('#sgp_zelanda').prop('checked','');
                $('#sgp_rusia').prop('checked','');
                $('#sgp_turquia').prop('checked','');
                $('#sgp_bielorrusia').prop('checked','');
                $('#sgp_ue').prop('checked','');
                $('#sgp_eeuu').prop('checked','');
                $('#sgp_canada').prop('checked','');
                $('#sgp_suiza').prop('checked','');
                $('#sgp_noruega').prop('checked','');
            }  
        });

        /***** Verificar la elaboracion de productos si es otros aparece Detallar *****/
        $('#elaboracion_otros').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                $('#div_elaboraciondetalle').fadeIn(1000);
            } else {  
                $('#div_elaboraciondetalle').fadeOut(1000);
            }  
        });
        
        /***** calcular los valores para pagar y aparecer criterios de origen *****/
        /*
        $('#check_can').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                //$('#total_ddjj').val(sumar);
                //$('#div_criterio_can').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                //$('#total_ddjj').val(restar);
                //$('#div_criterio_can').fadeOut(1000);
            }  
        });
        $('#check_ace22').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
                $('#div_criterio_ace22').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
                $('#div_criterio_ace22').fadeOut(1000);
            }  
        });
        $('#check_ace47').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
                $('#div_criterio_ace47').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
                $('#div_criterio_ace47').fadeOut(1000);
            }  
        });
        $('#check_arpar4').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
                $('#div_criterio_arpar4').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
                $('#div_criterio_arpar4').fadeOut(1000);
            }  
        });
        $('#check_aapag').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
                $('#div_criterio_aapag').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
                $('#div_criterio_aapag').fadeOut(1000);
            }  
        });
        $('#check_mercosur').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
                $('#div_criterio_mercosur').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
                $('#div_criterio_mercosur').fadeOut(1000);
            }  
        });
        $('#check_venezuela').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
                $('#div_criterio_venezuela').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
                $('#div_criterio_venezuela').fadeOut(1000);
            }  
        });
        $('#check_arampanama').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
            }  
        });
        
        $('#sgp_canada').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
                $('#div_criterio_sgpcanada').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
                $('#div_criterio_sgpcanada').fadeOut(1000);
            }  
        });
        $('#sgp_suiza').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
                $('#div_criterio_sgpsuiza').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
                $('#div_criterio_sgpsuiza').fadeOut(1000);
            }  
        });
        $('#sgp_noruega').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
                $('#div_criterio_sgpnoruega').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
                $('#div_criterio_sgpnoruega').fadeOut(1000);
            }  
        });
        $('#sgp_japon').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
                $('#div_criterio_sgpjapon').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
                $('#div_criterio_sgpjapon').fadeOut(1000);
            }  
        });
        $('#sgp_zelanda').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
                $('#div_criterio_sgpzelanda').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
                $('#div_criterio_sgpzelanda').fadeOut(1000);
            }  
        });
        $('#sgp_rusia').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
                $('#div_criterio_sgprusia').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
                $('#div_criterio_sgprusia').fadeOut(1000);
            }  
        });
        $('#sgp_turquia').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
                $('#div_criterio_sgpturquia').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
                $('#div_criterio_sgpturquia').fadeOut(1000);
            }  
        });
        $('#sgp_bielorrusia').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
                $('#div_criterio_sgpbielorrusia').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
                $('#div_criterio_sgpbielorrusia').fadeOut(1000);
            }  
        });
        $('#sgp_ue').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
                $('#div_criterio_sgpue').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
                $('#div_criterio_sgpue').fadeOut(1000);
            }  
        });
        $('#sgp_eeuu').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
                $('#div_criterio_sgpeeuu').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
                $('#div_criterio_sgpeeuu').fadeOut(1000);
            }  
        });
        $('#sgp_tp').change(function(){
            var marcado = $(this).prop('checked');
            if(marcado){
                var total=$('#total_ddjj').val();
                //var sumar=parseFloat(total)+60;
                $('#total_ddjj').val(sumar);
                $('#div_criterio_sgptp').fadeIn(1000);
            } else {  
                var total=$('#total_ddjj').val();
                //var restar=parseFloat(total)-60;
                $('#total_ddjj').val(restar);
                $('#div_criterio_sgptp').fadeOut(1000);
            }  
        });
        */
        $("#unidadmedida").kendoComboBox({
            placeholder:"Unidad de Medida",
            dataTextField: "descripcion",
            dataValueField: "id_unidad_medida",
            dataSource: {
                transport: {
                    read: {
                        dataType: "json",
                        url: "index.php?opcion=admUnidadMedida&tarea=listarUnidadMedida"
                    }
                }
            },
            change : function (e) {
                if (this.value() && this.selectedIndex === -1) { 
                    this.text('');
                }
            }
        });

        /*********** Tablas de la DDJJ ******/
        $("#tabla_comercializadores").kendoGrid({
            dataSource: dataComercializadores,
            editable: true,
            scrollable: false,
            resizable: true,
            selectable: "simple",
            columns: [
                { field: "razon_social", title: "Nombre o Razón social"},
                { field: "ci_nit", title: "CI o NIT"},
                { field: "domicilio", title: "Domicilio Legal"},
                { field: "representante_legal", title: "Repr. Legal"},
                { field: "direccion_fabrica", title: "Dirección de Fábrica"},
                { field: "telefono", title: "Teléfono"},
                { field: "precio_venta", title: "Precio Venta a Exportador"},
                { field: "unidad_medida", title: "Unidad de Medida", editor: UMedidaDropDownEditor},
                { field: "produccion_mensual", title: "Cap. Producción Mensual"}
            ]
        });
        
        $("#tabla_insumosnacionales").kendoGrid({
            dataSource: dataInsumosNacionales,
            editable: true,
            scrollable: false,
            resizable: true,
            selectable: "simple",
            columns: [
                { field: "descripcion", title: "Descripción"},
                { field: "fabricante", title: "Datos del Fabricante del Insumo"},
                { field: "telefono", title: "Teléfono del Fabricante"},
                { field: "valor", title: "Valor ($us)"}
            ]
        }).data("kendoGrid");
        
        $("#tabla_insumosimportados").kendoGrid({
            dataSource: dataInsumosImportados,
            editable: true,
            scrollable: false,
            resizable: true,
            selectable: "simple",
            columns: [
                { field: "descripcion", title: "Descripción"},
                { field: "nandina", title: "Código NANDINA"},
                { field: "pais", title: "País de Origen", editor: PaisDropDownEditor},
                { field: "productor", title: "Productor (Razón Social)"},
                { field: "cuenta_co", title: "Cuenta C.O.", editor: TienecoDropDownEditor},
                { field: "acuerdo", title: "Acuerdo Comercial", editor: AcuerdoDropDownEditor},
                { field: "cantidad", title: "Cantidad"},
                { field: "unidad_medida", title: "Unidad de Medida", editor: UMedidaDropDownEditor},
                { field: "valor", title: "Valor($us)"}
            ]
        });
        
        $("#combocapitulos").kendoDropDownList({
            optionLabel:"Escoge el Capítulo",
            dataTextField: "descripcion_total",
            dataValueField: "id_capitulo",
            dataSource: {
                transport: {
                    read: {
                        dataType: "json",
                        url: "index.php?opcion=admArancel&tarea=listarCapitulos"
                    }
                }
            },
            change : function (e) {
                if (this.value() && this.selectedIndex === -1) { 
                    this.text('');
                }
            }
        }).data("kendoDropDownList");

        $("#combonandina").kendoDropDownList({
            autoBind: false,
            cascadeFrom: "combocapitulos",
            optionLabel:"Clasificación Arancelaria - Descripción del Producto",
            dataTextField: "descripcion_total",
            dataValueField: "id_detalle_arancel",
            dataSource: {
                transport: {
                    read: {
                        dataType: "json",
                        url: "index.php?opcion=admArancel&tarea=listarArancelBoliviano"
                    }
                }
            },
            change : function (e) {
                if (this.value() && this.selectedIndex === -1) { 
                    this.text('');
                }
            }
        });
        //Combos para NALADISAS
        $("#naladisa93").kendoDropDownList({
            autoBind: false,
            cascadeFrom: "combocapitulos",
            optionLabel:"Escoger NALADISA 93",
            dataTextField: "descripcion_total",
            dataValueField: "id_detalle_arancel",
            dataSource: {
                transport: {
                    read: {
                        dataType: "json",
                        url: "index.php?opcion=admArancel&tarea=listarNaladisa&id_arancel=2"
                    }
                }
            },
            change : function (e) {
                if (this.value() && this.selectedIndex === -1) { 
                    this.text('');
                }
            }
        });

        $("#naladisa96").kendoDropDownList({
            autoBind: false,
            cascadeFrom: "combocapitulos",
            optionLabel:"ESCOGER NALADISA 96",
            dataTextField: "descripcion_total",
            dataValueField: "id_detalle_arancel",
            dataSource: {
                transport: {
                    read: {
                        dataType: "json",
                        url: "index.php?opcion=admArancel&tarea=listarNaladisa&id_arancel=3"
                    }
                }
            },
            change : function (e) {
                if (this.value() && this.selectedIndex === -1) { 
                    this.text('');
                }
            }
        });

        $("#naladisa2007").kendoDropDownList({
            autoBind: false,
            cascadeFrom: "combocapitulos",
            optionLabel:"Escoger NALADISA 2007",
            dataTextField: "descripcion_total",
            dataValueField: "id_detalle_arancel",
            dataSource: {
                transport: {
                    read: {
                        dataType: "json",
                        url: "index.php?opcion=admArancel&tarea=listarNaladisa&id_arancel=4"
                    }
                }
            },
            change : function (e) {
                if (this.value() && this.selectedIndex === -1) { 
                    this.text('');
                }
            }
        });

        $("#naladisa91").kendoDropDownList({
            autoBind: false,
            cascadeFrom: "combocapitulos",
            optionLabel:"Escoger NALADISA 91",
            dataTextField: "descripcion_total",
            dataValueField: "id_detalle_arancel",
            dataSource: {
                transport: {
                    read: {
                        dataType: "json",
                        url: "index.php?opcion=admArancel&tarea=listarNaladisa&id_arancel=5"
                    }
                }
            },
            change : function (e) {
                if (this.value() && this.selectedIndex === -1) { 
                    this.text('');
                }
            }
        });

        $("#naladi83").kendoDropDownList({
            autoBind: false,
            cascadeFrom: "combocapitulos",
            optionLabel:"ESCOGER NALADI 83",
            dataTextField: "descripcion_total",
            dataValueField: "id_detalle_arancel",
            dataSource: {
                transport: {
                    read: {
                        dataType: "json",
                        url: "index.php?opcion=admArancel&tarea=listarNaladisa&id_arancel=6"
                    }
                }
            },
            change : function (e) {
                if (this.value() && this.selectedIndex === -1) { 
                    this.text('');
                }
            }
        });
    });  

    function agregarfilacomercializador(){
        var tabla_comercializadores = $("#tabla_comercializadores").data("kendoGrid");
        tabla_comercializadores.addRow();
    }

    function agregarfilainsumosnacionales(){
        var tabla_insumosnacionales = $("#tabla_insumosnacionales").data("kendoGrid");
        tabla_insumosnacionales.addRow();
    }
    
    function agregarfilainsumosimportados(){
        var tabla_insumosimportados = $("#tabla_insumosimportados").data("kendoGrid");
        tabla_insumosimportados.addRow();
    }
    
    function eliminarfilacomercializador(){
        var tabla_comercializadores = $("#tabla_comercializadores").data("kendoGrid");
        var currentDataItem = tabla_comercializadores.dataItem(tabla_comercializadores.select());
        var dataRow = tabla_comercializadores.dataSource.getByUid(currentDataItem.uid);
        tabla_comercializadores.dataSource.remove(dataRow);
    }
    
    function eliminarfilainsumosnacionales(){
        var tabla_insumosnacionales = $("#tabla_insumosnacionales").data("kendoGrid");
        var currentDataItem = tabla_insumosnacionales.dataItem(tabla_insumosnacionales.select());
        var dataRow = tabla_insumosnacionales.dataSource.getByUid(currentDataItem.uid);
        tabla_insumosnacionales.dataSource.remove(dataRow);
    }
    
    function eliminarfilainsumosimportados(){
        var tabla_insumosimportados = $("#tabla_insumosimportados").data("kendoGrid");
        var currentDataItem = tabla_insumosimportados.dataItem(tabla_insumosimportados.select());
        var dataRow = tabla_insumosimportados.dataSource.getByUid(currentDataItem.uid);
        tabla_insumosimportados.dataSource.remove(dataRow);
    }

{literal}

$("#combo_fabricas").kendoComboBox({
    placeholder:"Seleccione la Fábrica donde se elabora la mercancía",
    dataTextField: "fabricas",
    dataValueField: "id_fabrica",
    filter: "contains",
    dataSource: {
        transport: {
            read: {
                dataType: "json",
                url: "index.php?opcion=admEmpresa&tarea=listarFabricas"
            }
        }
    },
    change : function (e) {
        if (this.value() && this.selectedIndex === -1) { 
            this.text('');
        }
    }
});

$("#otros_costos").kendoNumericTextBox();
$("#valor_fob").kendoNumericTextBox();
$("#valor_exwork").kendoNumericTextBox();
$("#produccion_mensual").kendoNumericTextBox();
//$("#otros_costos_ewk").kendoNumericTextBox();
{/literal}

</script>
       