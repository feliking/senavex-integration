            ...
            <div class="row-fluid  form" >
                ...
                <div class="row-fluid  form" >
                    <div class="span12" >
                        ...
                        {else}
                            {if $empresa->estado=='0'}
                                <b>el registro se encuentra en espera de revisión, puede realizar cambios</b>
                            {/if}
                            {if $empresa->estado=='9'}
                                <b>El Registro presenta Observaciones, realize los cambios señalados</b>
                            {/if}
                            {if $empresa->estado=='4'}
                                <b>Se envio el registro con actualizaciones para revision</b>
                            {/if}
                        {/if}
                    </div>
                </div>
                ...
                <div class="row-fluid  form" >
                    ...
                    <div class="span2" >
                         <input id="edicionempresa" {if $revision=='1'} type="button" {else} type="hidden"{/if}  value="Editar" class="k-primary" style="width:100%"/>
                    </div> 
                     <div class="span4" >
                    </div> 
                    <div class="span1 " >
...
aceptarmodifconf.bind("click", function(e){
    //window.open("index.php?"+$('#ruex').serialize()+'&habilitado='+habilitado+'&actualizacion='+actualizacion+'&editable={$editable}'+'&cobrar={$cobrar}'+'&afiliaciones='+multiSelect.value(), 'mywindow','width=400,height=200,toolbar=yes,location=yes,directories=yes,status=yes,menubar=yes,scrollbars=yes,copyhistory=yes,resizable=yes');
    aceptarmodifconf.enable(false);
    $.ajax({             
        type: 'post',
        url: 'index.php',
        data:$('#ruex').serialize()+'&habilitado='+habilitado+'&actualizacion='+actualizacion+'&editable={$editable}'+'&cobro={$cobro}'+'&afiliaciones='+multiSelect.value(),
...
function verificacheckboxnotificacion()
{
   ...
   else
   {
    ...
    if('{$editable}'== '0'){
        $('#chbx_nro_ruex').attr('readonly',true);
