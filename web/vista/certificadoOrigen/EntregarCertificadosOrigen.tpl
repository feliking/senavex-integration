<div class="row-fluid" class="fadein" >
    <div class="row-fluid  form" >
        <div id="entregarcertificadosorigen" class="fadein">
        </div>
    </div>
</div>
...
{literal}
 $(document).ready(function (){
    $("#entregarcertificadosorigen").kendoGrid({
        dataSource: {
            transport: {
                read: {
                   url: "index.php?opcion=admCertificado&tarea=listarEntregaCertificados",
                   dataType: "json"
                }
            },
            pageSize: 10
...
{  
    var gridco = $("#entregarcertificadosorigen").data("kendoGrid");
    var row = gridco.select();
    var data = gridco.dataItem(row);
    if(registroco==data.id_certificado_origen)
    {  
        anadir('Ver C.O.','admCertificado','mostrarCertificadoParaEntrega&id_certificado_origen='+data.id_certificado_origen);
    }
    else
    {
